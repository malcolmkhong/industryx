...
const NEWS_GENERATOR_URL =
  process.env.NEWS_GENERATOR_URL ??
...
describe("Cloudflare Workers Connectivity", () => {
  ...
  it("Market Tick worker exists (wrangler.toml found)", async () => {
    ...
    try {
      ...
      assert.ok(
        content.includes("markettick"),
        "wrangler.toml should have name=markettick",
      );
      assert.ok(
...
describe("API Routes Connectivity", () => {
  ...
  liveTest("POST /api/auth/bootstrap (canonical) returns non-5xx", async () => {
    try {
      const r = await fetch(`${BASE_URL}/api/auth/bootstrap`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ deviceId: "test-device-id-canonical" }),
        signal: AbortSignal.timeout(10000),
      });
      ...
      // 200/400/422 mean the route exists and is exercising the
      // canonical RPCs. 5xx would mean the deployment is broken.
      assert.ok(
        r.status < 500,
        `/api/auth/bootstrap returned ${r.status} — service error`,
      );
    } catch (err: unknown) {
      assert.fail(`/api/auth/bootstrap unreachable: ${(err as Error).message}`);
    }
  });
  ...
  liveTest("/api/market/tick is operational", async () => {
    try {
      ...
      console.log(
        `  Tick: ${body.tick}, Events: ${body.events}, Headlines: ${body.headlines}`,
      );
