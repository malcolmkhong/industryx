...
describe('Module: store/persistence', () => {
  ...
  it('exportSave/importSave roundtrip preserves state', () => {
    ...
    useGameStore.setState({ money: 99999 });

    // Import the saved state back
    const ok = getStore().importSave(save);
    expect(ok).toBe(true);
    expect(getStore().money).toBe(1000);
  });

  it('has debounced persist behavior (verify serialization)', () => {
    const save = getStore().exportSave();
    const decoded = JSON.parse(decodeURIComponent(atob(save)));
    expect(decoded).toHaveProperty('money');
    expect(decoded).toHaveProperty('gameTick');
    expect(decoded).toHaveProperty('resources');
    expect(decoded).toHaveProperty('buildings');
