...
const HOIST_BUILDING_DEFS = vi.hoisted(
  (): Record<string, Record<string, unknown>> => ({
    ...
    smelter: {
      ...
      costMultiplier: 1.5,
      baseProductionRate: 1,
      inputs: [{ resource: "iron", amount: 5 }],
      outputs: [{ resource: "ironPlate", amount: 10 }],
    },
    solarFarm: {
      name: "Solar Farm",
      category: "power",
    ...
    superFactory: {
      ...
      inputs: [{ resource: "steel", amount: 10 }],
      outputs: [{ resource: "robotics", amount: 5 }],
...
const HOIST_CONTRACT_TEMPLATES = vi.hoisted(() => [
  {
    name: "Supply Iron",
    type: "supply",
    requiredResources: [{ resource: "iron", amount: 50 }],
    timeLimit: 200,
    rewards: { money: 500 },
    icon: "",
    gameTier: 0,
...
vi.mock("@/lib/game/config/configCache", () => ({
  ...
  TRANSPORT_DEFS: {
    conveyorBelt: {
      ...
      icon: "",
      sortOrder: 1,
      baseCost: [{ resource: "money", amount: 50 }],
...
vi.mock("@/lib/game/migration/idMigration", () => ({
  migrateSaveBuildings: vi.fn((b) => b),
}));
...
describe("Module: utils/formatNumber", () => {
  ...
  it("formats integer < 1000 as string", () => {
  ...
  it("floors values >= 100", () => {
    expect(formatNumber(100.5)).toBe("100");
  });
  it("formats 1–99 with 1 decimal", () => {
    expect(formatNumber(1.5)).toBe("1.5");
  });
  it("formats 0–1 with 2 decimals", () => {
...
describe("Module: utils/hasUnlimitedStorage", () => {
  ...
  it("returns true when unlimitedStorage is completed", () => {
    expect(
      hasUnlimitedStorage([
        { completed: true, bonus: { type: "unlimitedStorage" as const } },
      ]),
    ).toBe(true);
  });
  it("returns false when project is incomplete", () => {
    expect(
      hasUnlimitedStorage([
        { completed: false, bonus: { type: "unlimitedStorage" as const } },
      ]),
    ).toBe(false);
  });
  it("returns false when different bonus type", () => {
    expect(
      hasUnlimitedStorage([
        { completed: true, bonus: { type: "buildingCostReduction" as const } },
      ]),
    ).toBe(false);
  });
});
...
describe("Module: utils/costCalculator", () => {
  describe("getBuildingCost()", () => {
    ...
    it("scales with count via costMultiplier", () => {
      expect(getBuildingCost("ironMine" as BuildingType, 1)).toBe(
        Math.floor(100 * 1.5),
      );
    });
    ...
    it("floors to minimum 1", () => {
      expect(
        getBuildingCost("ironMine" as BuildingType, 0, 1),
  ...
  describe("isBuildingUnlocked()", () => {
    it("returns true for building with no requirements", () => {
      expect(
        isBuildingUnlocked("ironMine" as BuildingType, [], {
          totalPrestiges: 0,
        }),
      ).toBe(true);
    });
    it("returns false for unknown building", () => {
      expect(
        isBuildingUnlocked("unknown" as BuildingType, [], {
          totalPrestiges: 0,
        }),
      ).toBe(false);
    });
...
describe("Module: services/leaderboardService", () => {
  ...
  it("addLeaderboardEntry sorts by score descending", () => {
    ...
    getStore().addLeaderboardEntry(e1);
    getStore().addLeaderboardEntry(e2);
...
describe("Module: services/newsService", () => {
  beforeEach(resetStore);
  it("getNewsLLMState returns object", () => {
    expect(getStore().getNewsLLMState()).toBeDefined();
  });
...
describe("Module: services/buildingService", () => {
  ...
  it("selectBuilding sets/clears", () => {
    getStore().selectBuilding("b1");
    expect(getStore().selectedBuilding).toBe("b1");
  ...
  it("upgradeBuilding no-op unknown", () => {
    getStore().upgradeBuilding("bad");
    expect(getStore().buildings).toHaveLength(0);
  });
  it("toggleBuilding flips active", async () => {
    const b = createMockBuilding("ironMine");
    useGameStore.setState({ buildings: [b] });
    await getStore().toggleBuilding(b.id);
    expect(getStore().buildings[0].active).toBe(false);
    await getStore().toggleBuilding(b.id);
    expect(getStore().buildings[0].active).toBe(true);
  });
  it("toggleBuilding no-op unknown", async () => {
    await getStore().toggleBuilding("bad");
  });
});
...
describe("Module: services/transportService", () => {
  ...
  it("toggleTransportLine flips active", () => {
    const line = {
      ...
      fromBuilding: "a",
      toBuilding: "b",
      carriesResource: "iron" as ResourceType,
      throughput: 10,
      maxThroughput: 30,
      active: true,
    };
  ...
  it("upgradeTransportLine no-op unknown", () => {
    getStore().upgradeTransportLine("bad");
  });
});
...
describe("Module: services/blueprintService", () => {
  ...
  it("exportBlueprint returns string", () => {
    ...
    expect(typeof getStore().exportBlueprint(getStore().blueprints[0].id)).toBe(
      "string",
    );
  });
  it("exportBlueprint empty for unknown", () => {
  ...
  it("importBlueprint rejects invalid", () => {
    expect(getStore().importBlueprint("bad")).toBe(false);
  });
  it("importBlueprint rejects >500 buildings", () => {
    expect(
      getStore().importBlueprint(
        btoa(
          encodeURIComponent(
            JSON.stringify({
              n: "Huge",
              b: Array(501).fill({ t: "ironMine", c: 1 }),
              t: [],
              v: 1,
            }),
          ),
...
describe("Module: services/contractService", () => {
  ...
  it("acceptContract adds", () => {
    ...
    getStore().acceptContract(c);
    expect(getStore().contracts).toHaveLength(1);
  });
  it("fulfillContract no-op unknown", () => {
    getStore().fulfillContract("bad");
  });
});
...
describe("Module: services/droneService", () => {
  ...
  it("sendDrone no-op with no drones", async () => {
    await getStore().sendDrone("m1", "d1");
  });
...
describe("Module: services/megaProjectService", () => {
  beforeEach(resetStore);
  it("startMegaProject no-op unknown", () => {
    getStore().startMegaProject("unknown" as any);
  });
  it("contributeToMegaProject no-op unknown", () => {
    getStore().contributeToMegaProject("unknown" as any);
  });
});
...
describe("Module: services/prestigeService", () => {
  beforeEach(resetStore);
  it("purchasePrestigeBonus no-op unknown", () => {
    getStore().purchasePrestigeBonus("bad");
  });
});
...
describe("Module: services/coreService", () => {
  ...
  it("setGameSpeed valid", async () => {
    await getStore().setGameSpeed(2);
    expect(getStore().gameSpeed).toBe(2);
  });
  it("setGameSpeed rejects invalid", async () => {
    await getStore().setGameSpeed(3);
    expect(getStore().gameSpeed).toBe(1);
  });
  it("setGameSpeed rejects 0", async () => {
...
describe("Module: services/storageService", () => {
  ...
  it("upgradeStorage rejected without money", () => {
    ...
    expect(getStore().resourceCapacity.iron).toBe(c);
  });
});
...
describe("Module: services/dailyRewardService", () => {
  ...
  // checkDailyLogin was removed — server-authoritative only.
  it("claimDailyReward handles fetch failure gracefully", async () => {
    await getStore().claimDailyReward(1);
  });
});
...
describe("Module: services/questService", () => {
  ...
  it("setTrackedQuest sets and clears", () => {
    getStore().setTrackedQuest("q1");
    expect(getStore().trackedQuest).toBe("q1");
