/**
 * TESTS: constants/initialState
 *
 * Tests for the initial game state shape.
 * Maps to target: constants/initialState.ts
...
const HOIST_WEEKLY_REWARDS = vi.hoisted(() => [
  { day: 1, type: 'money', amount: 100 },
  { day: 2, type: 'researchPoints', amount: 50 },
  { day: 3, type: 'resources', amount: 25 },
...
const HOIST_CONTRACT_TEMPLATES = vi.hoisted((): { name: string; type: string; requiredResources: { resource: string; amount: number }[]; timeLimit: number; rewards: { money: number }; icon: string; gameTier: number }[] => ([]));
...
describe('Module: constants/initialState', () => {
  ...
  it('market has entries', () => {
    expect(getStore().market.length).toBeGreaterThan(0);
    expect(getStore().market[0].resource).toBeDefined();
    expect(getStore().market[0].basePrice).toBeGreaterThan(0);
  });
});
