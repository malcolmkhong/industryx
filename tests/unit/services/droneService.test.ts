...
import type { ResourceType, BuildingInstance } from '@/lib/game/shared/types/types';
...
function createMockBuilding(type: string, overrides: Partial<BuildingInstance> = {}): BuildingInstance {
  return { id: crypto.randomUUID(), type: type as any, level: 1, active: true, efficiency: 1, placedAt: 0, ...overrides };
}

describe('Module: services/droneService', () => {
  beforeEach(() => { resetStore(); vi.clearAllMocks(); });

  it('buyDrone adds drone to fleet', () => {
    const n = getStore().drones.fleet.length;
    getStore().buyDrone();
    expect(getStore().drones.fleet.length).toBe(n + 1);
  });

  it('buyDrone does nothing with no money', () => {
  ...
  it('sendDrone does nothing with no drone', async () => {
    await expect(getStore().sendDrone('m1', 'd1')).resolves.toBeUndefined();
  });

  it('upgradeDrone upgrades drone speed', () => {
    const droneId = getStore().drones.fleet[0].id;
    const before = getStore().drones.fleet[0].speedLevel;
    getStore().upgradeDrone(droneId, 'speed');
    expect(getStore().drones.fleet[0].speedLevel).toBe(before + 1);
  });
});
