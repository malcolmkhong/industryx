...
import type { Contract, ResourceType } from '@/lib/game/shared/types/types';
...
describe('Module: services/contractService', () => {
  ...
  it('acceptContract adds to contracts array', () => {
    const contract: Contract = {
      id: 'c1', name: 'Test Contract', type: 'supply', description: '',
      requiredResources: [{ resource: 'iron', amount: 10 }],
      timeLimit: 100, timeRemaining: 100,
      reward: { money: 500, researchPoints: 10, corporationPoints: 0 },
    ...
    expect(getStore().contracts[0].name).toBe('Test Contract');
  });

  it('fulfillContract does nothing for non-existent contract', () => {
    const contractsBefore = getStore().contracts.length;
    getStore().fulfillContract('nonexistent-id');
    expect(getStore().contracts.length).toBe(contractsBefore);
  });
});
