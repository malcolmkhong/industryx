...
describe('Module: services/storageService', () => {
  ...
  it('upgradeStorage increases capacity', () => {
    const beforeCapacity = getStore().resourceCapacity.iron;
    const beforeLevel = getStore().storageUpgradeLevels.iron;
    getStore().upgradeStorage('iron', 1);
    expect(getStore().resourceCapacity.iron).toBeGreaterThan(beforeCapacity);
    expect(getStore().storageUpgradeLevels.iron).toBe(beforeLevel + 1);
  });

  it('upgradeStorage does nothing with no money', () => {
    useGameStore.setState({ money: 0 });
    const beforeCapacity = getStore().resourceCapacity.iron;
    getStore().upgradeStorage('iron', 1);
    expect(getStore().resourceCapacity.iron).toBe(beforeCapacity);
  });
});
