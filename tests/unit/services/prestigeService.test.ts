...
describe('Module: services/prestigeService', () => {
  ...
  it('purchasePrestigeBonus does nothing when bonus not found', () => {
    ...
    getStore().purchasePrestigeBonus('nonexistent-bonus');
    expect(getStore().prestigeState.corporationPoints).toBe(before);
  });
});
