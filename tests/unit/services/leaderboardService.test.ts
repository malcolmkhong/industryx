 ...
 *
 * Tests leaderboard actions: addLeaderboardEntry.
 *
 * Target: services/leaderboardService.ts (extracted from store.ts)
 * Imports: @/lib/game/state/store (current monolithic store)
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import type { LeaderboardEntry } from '@/lib/game/shared/types/types';
...
describe('services/leaderboardService', () => {
  ...
  it('adds entry to leaderboardEntries', () => {
    const entry: LeaderboardEntry = {
      id: 'a', corporationName: 'Corp A', score: 100, rank: 1,
      buildingsBuilt: 0, researchCompleted: 0, contractsCompleted: 0,
      totalMoneyEarned: 0, playTime: 0, prestigeCount: 0, rankName: '', achievedAt: 0,
    };
    getStore().addLeaderboardEntry(entry);
    expect(getStore().leaderboardEntries).toHaveLength(1);
    expect(getStore().leaderboardEntries[0].id).toBe('a');
  });

  it('sorts by score descending', () => {
    const low: LeaderboardEntry = {
      id: 'a', corporationName: 'Low', score: 100, rank: 2,
      buildingsBuilt: 0, researchCompleted: 0, contractsCompleted: 0,
      totalMoneyEarned: 0, playTime: 0, prestigeCount: 0, rankName: '', achievedAt: 0,
    };
    const high: LeaderboardEntry = {
      id: 'b', corporationName: 'High', score: 200, rank: 1,
      buildingsBuilt: 0, researchCompleted: 0, contractsCompleted: 0,
      totalMoneyEarned: 0, playTime: 0, prestigeCount: 0, rankName: '', achievedAt: 0,
    };
    getStore().addLeaderboardEntry(low);
    getStore().addLeaderboardEntry(high);
    expect(getStore().leaderboardEntries[0].score).toBe(200);
    expect(getStore().leaderboardEntries[1].score).toBe(100);
  });

  it('assigns rank based on position', () => {
    const entries: LeaderboardEntry[] = [
      { id: 'a', corporationName: 'A', score: 300, rank: 0, buildingsBuilt: 0, researchCompleted: 0, contractsCompleted: 0, totalMoneyEarned: 0, playTime: 0, prestigeCount: 0, rankName: '', achievedAt: 0 },
      { id: 'b', corporationName: 'B', score: 200, rank: 0, buildingsBuilt: 0, researchCompleted: 0, contractsCompleted: 0, totalMoneyEarned: 0, playTime: 0, prestigeCount: 0, rankName: '', achievedAt: 0 },
      { id: 'c', corporationName: 'C', score: 100, rank: 0, buildingsBuilt: 0, researchCompleted: 0, contractsCompleted: 0, totalMoneyEarned: 0, playTime: 0, prestigeCount: 0, rankName: '', achievedAt: 0 },
    ];
    for (const e of entries) getStore().addLeaderboardEntry(e);
    expect(getStore().leaderboardEntries[0].rank).toBe(1);
    expect(getStore().leaderboardEntries[1].rank).toBe(2);
    expect(getStore().leaderboardEntries[2].rank).toBe(3);
  });
});
