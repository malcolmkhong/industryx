 ...
 *
 * Tests building actions: buildBuilding, upgradeBuilding, toggleBuilding, selectBuilding.
 *
 * Target: services/buildingService.ts (extracted from store.ts)
 * Imports: @/lib/game/state/store (current monolithic store)
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import type { BuildingType, BuildingInstance } from '@/lib/game/shared/types/types';
...
function createMockBuilding(type: string, overrides: Partial<BuildingInstance> = {}): BuildingInstance {
  return {
    id: crypto.randomUUID(), type: type as BuildingType, level: 1,
...
describe('services/buildingService', () => {
  ...
  it('buildBuilding rejects insufficient funds', async () => {
    useGameStore.setState({ money: 10 });
    await getStore().buildBuilding('ironMine' as BuildingType);
    expect(getStore().buildings).toHaveLength(0);
  });

  it('buildBuilding rejects max buildings (501)', async () => {
    const buildings = Array(501).fill(null).map((_, i) => ({
      id: `b${i}`, type: 'ironMine' as BuildingType, level: 1, active: true, efficiency: 1, placedAt: 0,
    }));
    useGameStore.setState({ money: 99999, buildings });
    await getStore().buildBuilding('ironMine' as BuildingType);
    expect(getStore().buildings).toHaveLength(501);
  });
  ...
  it('upgradeBuilding does nothing for non-existent', () => {
    getStore().upgradeBuilding('non-existent-id');
    expect(getStore().buildings).toHaveLength(0);
  });

  it('toggleBuilding toggles active state', async () => {
    const b = createMockBuilding('ironMine');
  ...
  it('selectBuilding sets selectedBuilding', () => {
    getStore().selectBuilding('b1');
    expect(getStore().selectedBuilding).toBe('b1');
  });

  it('selectBuilding null clears selection', () => {
    getStore().selectBuilding('b1');
