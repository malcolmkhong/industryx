 ...
 * RANK SERVICE TESTS
 *
 * Tests rank actions: getCurrentRank, getPlayerGameTier, divergesFromExpected.
 *
 * Target: services/rankService.ts (extracted from store.ts)
...
function createMockBuilding(type: string): BuildingInstance {
  return { id: crypto.randomUUID(), type: type as BuildingType, level: 1, active: true, efficiency: 1, placedAt: 0 };
}
...
describe('services/rankService', () => {
  ...
  it('divergesFromExpected returns false when money within range', () => {
    useGameStore.setState({ money: 1000 });
    expect(getStore().divergesFromExpected(1100)).toBe(false);
  });

  it('divergesFromExpected returns true when money exceeds by >10%', () => {
