 ...
 *
 * Tests payout actions: collectPayout, toggleAutoCollect.
 *
 * Target: services/payoutService.ts (extracted from store.ts)
...
describe('services/payoutService', () => {
  beforeEach(() => { vi.clearAllMocks(); resetStore(); });

  it('toggleAutoCollect toggles autoCollect', () => {
  ...
  it('collectPayout adds pendingPayout to money', () => {
  ...
  it('collectPayout resets pendingPayout to 0', () => {
    useGameStore.setState({ pendingPayout: 500 });
    getStore().collectPayout();
    expect(getStore().pendingPayout).toBe(0);
  });

  it('collectPayout does nothing when no pending payout', () => {
    const moneyBefore = getStore().money;
    getStore().collectPayout();
