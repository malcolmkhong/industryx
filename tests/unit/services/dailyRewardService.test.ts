...
describe('Module: services/dailyRewardService', () => {
  ...
  it('checkDailyLogin calls the API', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        success: true,
        reward: { type: 'money', amount: 100 },
        streak: { currentStreak: 1, longestStreak: 1, lastLoginDate: new Date().toISOString().split('T')[0], totalLogins: 1 },
        alreadyClaimed: false,
      }),
    });
    await getStore().checkDailyLogin();
    expect(mockFetch).toHaveBeenCalledWith('/api/game/rewards/daily', expect.any(Object));
  });

  it('claimDailyReward calls the API', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        success: true,
        reward: { type: 'money', amount: 100, day: 1 },
        streak: { currentStreak: 1, longestStreak: 1, lastLoginDate: new Date().toISOString().split('T')[0], totalLogins: 1 },
      }),
    });
