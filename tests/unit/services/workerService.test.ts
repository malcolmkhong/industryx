...
describe('Module: services/workerService', () => {
  beforeEach(() => { resetStore(); vi.clearAllMocks(); });

  it('levelUpWorker is a no-op', () => {
    const workersBefore = getStore().workers.length;
    getStore().levelUpWorker('nonexistent');
    expect(getStore().workers.length).toBe(workersBefore);
  });
});
