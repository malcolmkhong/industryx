 ...
 * SAVE SERVICE TESTS
 *
 * Tests save/load actions: exportSave, importSave, resetGame.
 *
 * Target: services/saveService.ts (extracted from store.ts)
...
describe('services/saveService', () => {
  ...
  it('exportSave is valid base64', () => {
    const save = getStore().exportSave();
    expect(() => atob(save)).not.toThrow();
  });

  it('importSave imports valid save', () => {
    const save = getStore().exportSave();
  ...
  it('importSave rejects invalid string', () => {
    expect(getStore().importSave('bad')).toBe(false);
  });

  it('importSave rejects money > 1e12', () => {
    const badSave = btoa(encodeURIComponent(JSON.stringify({
      money: 1e13, gameTick: 0, resources: {} as Record<ResourceType, number>,
      buildings: [], _version: 20,
    })));
    expect(getStore().importSave(badSave)).toBe(false);
  });

  it('importSave rejects negative money', () => {
    const badSave = btoa(encodeURIComponent(JSON.stringify({
      money: -1, gameTick: 0, resources: {} as Record<ResourceType, number>,
      buildings: [], _version: 20,
    })));
    expect(getStore().importSave(badSave)).toBe(false);
  });

  it('importSave rejects Infinity money', () => {
    const badSave = btoa(encodeURIComponent(JSON.stringify({
      money: Infinity, gameTick: 0, resources: {} as Record<ResourceType, number>,
      buildings: [], _version: 20,
    })));
    expect(getStore().importSave(badSave)).toBe(false);
  });
  ...
  it('importSave rejects >500 buildings', () => {
    const buildings = Array(501).fill(null).map((_, i) => ({
      id: `b${i}`, type: 'ironMine', level: 1, active: true, efficiency: 1, placedAt: 0,
    }));
    const badSave = btoa(encodeURIComponent(JSON.stringify({
      money: 1000, gameTick: 0, resources: { iron: 0 } as Record<string, number>,
      buildings, _version: 20,
    })));
    expect(getStore().importSave(badSave)).toBe(false);
  });
