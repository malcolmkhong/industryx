 ...
 *
 * Tests notification actions: addNotification, markNotificationRead,
 * markAllNotificationsRead, clearNotifications.
 *
 * Target: services/notificationService.ts (extracted from store.ts)
...
const HOIST_WEEKLY_REWARDS = vi.hoisted(() => [
  ...
  { day: 3, type: 'resources', amount: 25, resource: 'iron' as const },
...
describe('services/notificationService', () => {
  ...
  it('addNotification adds to notifications array', () => {
    getStore().addNotification('info', 'Test notification');
    expect(getStore().notifications).toHaveLength(1);
  });

  it('addNotification caps at 30', () => {
    for (let i = 0; i < 35; i++) {
      getStore().addNotification('info', `Notification ${i}`);
    }
    expect(getStore().notifications.length).toBeLessThanOrEqual(30);
  });

  it('markNotificationRead marks as read', () => {
    getStore().addNotification('success', 'Mark me');
  ...
  it('clearNotifications empties array', () => {
    getStore().addNotification('info', 'To be cleared');
    getStore().clearNotifications();
    expect(getStore().notifications).toEqual([]);
  });

  it('notification has id, type, message, read, gameTick fields', () => {
    getStore().addNotification('success', 'Field check');
    const n = getStore().notifications[0];
    expect(n).toHaveProperty('id');
    expect(n).toHaveProperty('type');
    expect(n).toHaveProperty('message');
    expect(n).toHaveProperty('read');
    expect(n).toHaveProperty('gameTick');
    expect(n.message).toBe('Field check');
