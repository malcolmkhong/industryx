 ...
 *
 * Tests blueprint actions: saveBlueprint, deleteBlueprint, renameBlueprint,
 * exportBlueprint, importBlueprint.
 *
 * Target: services/blueprintService.ts (extracted from store.ts)
...
describe("services/blueprintService", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resetStore();
  });

  it("saveBlueprint creates blueprint", () => {
    getStore().saveBlueprint("Test Blueprint");
    expect(getStore().blueprints).toHaveLength(1);
    expect(getStore().blueprints[0].name).toBe("Test Blueprint");
  });

  it("deleteBlueprint removes blueprint", () => {
    getStore().saveBlueprint("To Delete");
    const id = getStore().blueprints[0].id;
    getStore().deleteBlueprint(id);
    expect(getStore().blueprints).toHaveLength(0);
  });

  it("renameBlueprint renames blueprint", () => {
    getStore().saveBlueprint("Original");
    const id = getStore().blueprints[0].id;
    getStore().renameBlueprint(id, "Renamed");
    expect(getStore().blueprints[0].name).toBe("Renamed");
  });

  it("exportBlueprint returns base64 string", () => {
  ...
  it("exportBlueprint returns empty string for non-existent", () => {
    expect(getStore().exportBlueprint("non-existent-id")).toBe("");
  });

  it("importBlueprint accepts valid code", () => {
    // Build a blueprint that carries at least one building + transport
    // entry — saveBlueprint snapshots the current state, so we have
    // to seed the store first. An empty blueprint is rejected by
    // validateBlueprint() with reason: 'empty' (intentional UX).
    //
    // buildBuilding is server-authoritative (it calls
    // validateActionWithServer) so we can't drive it through the mock
    // store. We seed the state directly instead.
    ...
    getStore().saveBlueprint("Source");
    const code = getStore().exportBlueprint(getStore().blueprints[0].id);
    const result = getStore().importBlueprint(code);
    expect(result).toBe(true);
  });

  it("importBlueprint rejects invalid code", () => {
    expect(getStore().importBlueprint("invalid-code")).toBe(false);
  });
  ...
  it("importBlueprint rejects >500 buildings", () => {
    ...
    expect(getStore().importBlueprint(badCode)).toBe(false);
  });
