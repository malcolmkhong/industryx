 ...
 *
 * Tests transport actions: buildTransportLine, upgradeTransportLine, toggleTransportLine.
 *
 * Target: services/transportService.ts (extracted from store.ts)
...
describe('services/transportService', () => {
  ...
  it('toggleTransportLine toggles active', () => {
    const line = {
      id: 't1', type: 'conveyorBelt', level: 1, fromBuilding: 'a', toBuilding: 'b',
      carriesResource: 'iron' as ResourceType, throughput: 10, maxThroughput: 30, active: true,
    };
  ...
  it('upgradeTransportLine does nothing for non-existent line', () => {
    const result = getStore().upgradeTransportLine('non-existent');
    expect(result).toBeUndefined();
    expect(getStore().transportLines).toHaveLength(0);
  });

  it('buildTransportLine adds line', () => {
    useGameStore.setState({ money: 999999 });
    getStore().buildTransportLine('conveyorBelt' as any, 'b1', 'b2', 'iron' as ResourceType);
    expect(getStore().transportLines).toHaveLength(1);
    expect(getStore().transportLines[0].carriesResource).toBe('iron');
  });
});
