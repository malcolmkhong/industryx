...
import type { ResourceType, MegaProjectType } from '@/lib/game/shared/types/types';
...
describe('Module: services/megaProjectService', () => {
  ...
  it('startMegaProject does nothing for unknown type', () => {
    getStore().startMegaProject('unknown' as MegaProjectType);
    expect(getStore().megaProjects).toEqual([]);
  });

  it('contributeToMegaProject does nothing for unknown type', () => {
    getStore().contributeToMegaProject('unknown' as MegaProjectType);
    expect(getStore().megaProjects).toEqual([]);
  });
});
