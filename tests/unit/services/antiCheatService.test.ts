...
describe('Module: services/antiCheatService', () => {
  ...
  it('returns false when money within expected range', () => {
    useGameStore.setState({ money: 1000 });
    expect(getStore().divergesFromExpected(1100)).toBe(false);
  });

  it('returns false when serverComputedMax is 0', () => {
    useGameStore.setState({ money: 1000 });
    expect(getStore().divergesFromExpected(0)).toBe(false);
  });

  it('returns false when serverComputedMax is negative', () => {
    useGameStore.setState({ money: 1000 });
    expect(getStore().divergesFromExpected(-1)).toBe(false);
  });

  it('returns true when money exceeds by >10%', () => {
