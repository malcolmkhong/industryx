...
describe('Module: services/researchService', () => {
  beforeEach(() => { resetStore(); vi.clearAllMocks(); });

  it('startResearch is defined as an async function', () => {
    expect(getStore().startResearch).toBeDefined();
    expect(typeof getStore().startResearch).toBe('function');
    expect(getStore().startResearch.constructor.name).toBe('AsyncFunction');
  });

  it('exists on the store', () => {
    expect('startResearch' in getStore()).toBe(true);
  });
});
