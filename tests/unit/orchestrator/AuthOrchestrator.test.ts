...
interface DepsBundle {
  ...
  mocks: {
    ...
    recoverByDevice: ReturnType<typeof vi.fn>;
    claimGuest: ReturnType<typeof vi.fn>;
...
function buildDeps(overrides: Partial<AuthOrchestratorDeps> = {}): DepsBundle {
  ...
  const m = {
    ...
    recoverByDevice: vi.fn(async () => ({ recovered: false, userId: null })),
    claimGuest: vi.fn(async () => ({ ok: true, error: null })),
    quickstart: vi.fn(async () => ({ userId: 'anon-new', error: null })),
  ...
  return {
    ...
    mocks: {
      ...
      recoverByDevice: deps.recoverByDevice as unknown as ReturnType<typeof vi.fn>,
      claimGuest: deps.claimGuest as unknown as ReturnType<typeof vi.fn>,
      quickstart: deps.quickstart as unknown as ReturnType<typeof vi.fn>,
      signInWithOAuth: deps.signInWithOAuth as unknown as ReturnType<typeof vi.fn>,
      registerDevice: deps.registerDevice as unknown as ReturnType<typeof vi.fn>,
      signOutSupabase: deps.signOutSupabase as unknown as ReturnType<typeof vi.fn>,
      disableServerValidation: deps.disableServerValidation as unknown as ReturnType<typeof vi.fn>,
      initServerValidation: deps.initServerValidation as unknown as ReturnType<typeof vi.fn>,
      onReady: deps.onReady as unknown as ReturnType<typeof vi.fn>,
      onIdentityChanged: deps.onIdentityChanged as unknown as ReturnType<typeof vi.fn>,
      ...
      startLoginPrompts: deps.startLoginPrompts as unknown as ReturnType<typeof vi.fn>,
      stopLoginPrompts: deps.stopLoginPrompts as unknown as ReturnType<typeof vi.fn>,
      getDeviceId: deps.getDeviceId as unknown as ReturnType<typeof vi.fn>,
      onAuthStateChange: deps.onAuthStateChange as unknown as ReturnType<typeof vi.fn>,
    },
  };
}
...
describe('AuthOrchestrator', () => {
  ...
  describe('startup()', () => {
    ...
    it('with anon session — onReady + startLoginPrompts wired', async () => {
      ...
      expect(mocks.startLoginPrompts).toHaveBeenCalledWith(expect.any(Function));
      cleanup();
    });
    ...
    it('with no session, no recovery — NO_RECOVERY + anon + init; no claim', async () => {
      const { deps, mocks } = buildDeps();
      orch.attach(deps);
      const events = captureEvents(orch);
      ...
      expect(mocks.recoverByDevice).toHaveBeenCalledWith('device-1', null);
      expect(events.some((e) => e.type === 'NO_RECOVERY')).toBe(true);
      expect(mocks.quickstart).toHaveBeenCalledWith('device-1', null, null);
      expect(mocks.claimGuest).not.toHaveBeenCalled();
      cleanup();
    });

    it('with no session + recovery — RECOVERED + anon + init + claim', async () => {
      const { deps, mocks } = buildDeps({
        recoverByDevice: vi.fn(async () => ({
          recovered: true,
          userId: 'old-anon',
        })),
      });
      ...
      expect(mocks.quickstart).toHaveBeenCalledWith('device-1', null, 'old-anon');
      expect(mocks.claimGuest).toHaveBeenCalledWith('old-anon', 'device-1');
      cleanup();
    });
    ...
    it('quickstart capacity_full → WAITLIST_REQUIRED dispatched', async () => {
      ...
      expect(mocks.quickstart).toHaveBeenCalled();
      expect(events.some((e) => e.type === 'WAITLIST_REQUIRED')).toBe(true);
      cleanup();
    });

    it('forwards fingerprintHash to recoverByDevice + quickstart', async () => {
      const { deps, mocks } = buildDeps();
      orch.attach(deps);

      const cleanup = await orch.startup('fp-hash-xyz');

      expect(mocks.recoverByDevice).toHaveBeenCalledWith('device-1', 'fp-hash-xyz');
      expect(mocks.quickstart).toHaveBeenCalledWith('device-1', 'fp-hash-xyz', null);
      cleanup();
    });
  ...
  describe('onAuthStateChange-driven transitions', () => {
    ...
    it('null→anon via SIGNED_IN (recovery path) — fires onReady once, subsequent same userId no-op', async () => {
      ...
      handlers.forEach((h) => h(makeSession({ userId: 'anon-new', isAnonymous: true })));
  ...
  describe('signOut()', () => {
    it('dispatches SIGN_OUT, clears state, calls onSignedOut + resetMerge + stopLoginPrompts + disableServerValidation', async () => {
      ...
      orch.setState({ userId: 'before', status: 'ready', identity: 'authenticated' });
  ...
  describe('signInWithOAuth()', () => {
    it('success — dispatches OAUTH_CALLBACK then OAUTH_SUCCESS; returns null', async () => {
      ...
      const error = await orch.signInWithOAuth('google', 'https://app/callback');

      expect(error).toBeNull();
      expect(events.findIndex((e) => e.type === 'OAUTH_CALLBACK')).toBeLessThan(
        events.findIndex((e) => e.type === 'OAUTH_SUCCESS'),
      );
      expect(mocks.signInWithOAuth).toHaveBeenCalledWith('google', 'https://app/callback');
    });
    ...
    it('failure — dispatches OAUTH_CALLBACK then OAUTH_FAILURE; returns error string', async () => {
      ...
      const error = await orch.signInWithOAuth('github', 'https://app/callback');
  ...
  describe('runPostOAuth()', () => {
    it('calls registerDevice with current deviceId + fingerprint', async () => {
      ...
      expect(mocks.registerDevice).toHaveBeenCalledWith('device-1', null, 'fp-hash');
    });
