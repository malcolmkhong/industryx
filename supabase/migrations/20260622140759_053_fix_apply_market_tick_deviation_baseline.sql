...
-- (last successful tick: 2026-06-20 08:06:44, current tick frozen at 2195).
-- Worker POST returned: "Price change for voidEnergy exceeds 50% in single tick"
--
-- Fix: build a map of previous currentPrice per resource, then validate
-- per-tick delta against that map. Fall back to basePrice check only if
-- the resource is not in the previous tick (first occurrence).
--
-- marketEngine.js caps per-tick delta at SPIKE_CAP = 0.40 (40%). We keep
-- the 50% ceiling as a defensive ceiling slightly above the engine cap,
-- and additionally enforce a 1,000,000 absolute price ceiling per tick
-- to prevent runaway inflation regardless of baseline.
--
-- Date: 2026-06-22
-- Discovered via manual cron trigger test of markettick worker.
...
BEGIN
  ...
  -- (fall back to basePrice if resource is brand new this tick).
  FOR v_price_entry IN SELECT * FROM jsonb_array_elements(p_prices) LOOP
    ...
    -- Look up previous currentPrice for this resource
    ...
    -- Fall back to basePrice only when resource is new (no previous entry).
  ...
  SET tick = p_tick, prices = p_prices, volatility = p_volatility, circuit_breakers = p_breakers, updated_at = now()
  WHERE id = 1;
  ...
  FOR v_event_entry IN SELECT * FROM jsonb_array_elements(p_events) LOOP
    ...
    INSERT INTO game_config_market_history (resource_id, base_price, market_phase, game_tick) VALUES (
      ...
      COALESCE((v_event_entry->'context'->>'newPrice')::NUMERIC, (v_event_entry->'context'->>'oldPrice')::NUMERIC, 0),
      CASE WHEN v_event_entry->>'type' = 'price_move' THEN COALESCE(v_event_entry->'context'->>'cause', 'price_move')
           WHEN v_event_entry->>'type' = 'breaker' THEN 'circuit_breaker'
           ELSE COALESCE(v_event_entry->>'type', 'unknown') END,
      p_tick
    );
    v_history_count := v_history_count + 1;
  END LOOP;

  -- FIX: explicit WHERE clause (BUG-042). Prior `DELETE FROM market_player_pressure;` was rejected at runtime.
...
-- Re-grant execute to service_role (per migration 052 convention)
