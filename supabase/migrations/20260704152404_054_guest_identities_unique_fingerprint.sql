...
--
-- Pre-flight: deduplicate any existing same-fingerprint rows BEFORE creating index.
--   Strategy: keep the EARLIEST active row, supersede all later ones.
--
-- Note: 'unknown' fingerprint is the fallback when FingerprintJS fails.
--   We allow multiple 'unknown' rows because they have no useful signal.
--   The unique constraint will be on (fingerprint) WHERE fingerprint != 'unknown'
...
DECLARE
  dup_record RECORD;
BEGIN
  -- For each fingerprint that has >1 active rows: keep earliest, supersede rest.
  FOR dup_record IN
    SELECT fingerprint
    ...
    HAVING COUNT(*) > 1
  LOOP
    -- Mark all-but-the-earliest as superseded by the surviving row's user_id.
...
-- ============================================================================
-- 4. ROLLBACK NOTES (if you need to undo this migration)
-- ============================================================================
-- DROP INDEX IF EXISTS public.idx_guest_identities_one_active_per_fingerprint;
-- Note: dedup is destructive. Roll forward instead of backward.
