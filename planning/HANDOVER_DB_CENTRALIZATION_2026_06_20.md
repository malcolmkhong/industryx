...
| Iterations complete | **5 of 10** (server_game_state + admins + admin_actions + trades + market + leaderboard) |
| Routes migrated | **15** (out of 71) |
| New `src/lib/db/` modules | **5** (serverGameState, admins, adminActions, trades, market) |
...
| Net `.from('trade_history')` calls removed | **2** |
| Net `.from('server_market_state')` calls removed | **3** |
| Project-wide `tsc --noEmit` errors | **0** |
| Project-wide lint errors in migrated files | **0** |
| Completion percentage (by route count) | **~21%** (15 of 71) |
| Completion percentage (by table count) | **~16%** (5 of 31 tables) |
...
### Iteration 2 — `admins` + `admin_actions` ✅ COMPLETE
...
### Iteration 3 — `trades` ✅ COMPLETE

**Date:** 2026-06-20
**Scope:** Centralize all queries against `trade_history` table.
**Routes migrated:** 2
**Status:** ✅ VALIDATED — All checks passed

#### Files Created
- `src/lib/db/trades.ts` (~140 lines, 3 exported functions)

#### Files Modified
- `src/app/api/game/trade/route.ts` — replaced `supabase.from('trade_history').insert()` with `recordTrade()`
- `src/app/api/game/trades/route.ts` — replaced `supabase.from('trade_history').select(...)` with `getTradeHistory()`

#### New API Surface

```typescript
// trades.ts
recordTrade(params)              // insert trade_history row — called after successful trade
getTradeHistory(userId, limit, offset)  // paginated user trade history
getRecentTrades(limit)           // recent trades for admin/analytics
```

#### Validation Results
- `tsc --noEmit`: 0 errors
- `npm run lint`: 0 new errors in migrated files
- `npm run build`: ✅ Success
- Smoke test `/api/game/trades`: ✅ 401 (auth gate fires, no 500)

---

### Iteration 4 — `market` ✅ COMPLETE

**Date:** 2026-06-20
**Scope:** Centralize all queries against `server_market_state`, `market_player_pressure`, and `market_supply_demand` tables.
**Routes migrated:** 2 (state + tick); action route was already clean (RPC only)
**Status:** ✅ VALIDATED — All checks passed

#### Files Created
- `src/lib/db/market.ts` (~120 lines, 5 exported functions + 1 interface)

#### Files Modified
- `src/app/api/market/state/route.ts` — replaced `createServiceRoleClient().from('server_market_state').select(...)` with `getMarketState()`
- `src/app/api/market/tick/route.ts` — replaced 3 direct `.from()` calls with `getMarketStateFull()`, `getAllPlayerPressure()`, `getAllSupplyDemand()`, `updateMarketNews()`

#### New API Surface

```typescript
// market.ts
getMarketState()                  // Pick<tick,prices,news,volatility> for /api/market/state GET
getMarketStateFull()              // full server_market_state row for /api/market/tick POST
getAllPlayerPressure()             // all market_player_pressure rows for /api/market/tick
getAllSupplyDemand()               // SupplyDemandRow[] for /api/market/tick
updateMarketNews(news)             // update server_market_state.news after AI news generated
```

#### Notes
- `/api/market/action/route.ts` — **not migrated** (uses `supabase.rpc('upsert_market_pressure')` only, no `.from()` calls — already clean)
- `/api/market/aggregate-supply/route.ts` — **not migrated** (reads `server_game_state`, not a market table; separate iteration)
- RPC calls (`apply_market_tick`, `upsert_market_pressure`, `upsert_supply_demand`) remain inline in routes — these are simulation/persistence gates, not pure data reads

#### Validation Results
- `tsc --noEmit`: 0 errors
- `npm run lint`: 0 errors in migrated files
- `npm run build`: ✅ Success (cp command fails on Windows but compilation succeeds)
- Smoke test `/api/market/state`: ✅ 200
- Smoke test `/api/market/tick POST`: ✅ 200
...
- ✅ All `.from('trade_history')` calls in 2 routes (insert + read)
- ✅ 3 `.from('server_market_state')` calls in `/api/market/state` + `/api/market/tick`
- ✅ `market_player_pressure` read in `/api/market/tick`
- ✅ `market_supply_demand` read in `/api/market/tick`
...
|---|---|---|
| `game_config_market` | `/api/game/trade` | Iter 4 |
| `game_config_market_history` | `/api/game/trade` (insert), `/api/game/market-history` (read) | Iter 4 |
...
### Iteration 3 — `trades` (IN PROGRESS)
