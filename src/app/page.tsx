    ...
    <TooltipProvider>
      ...
      <div className="h-screen flex flex-col bg-[#0a0e17] text-gray-100 overflow-hidden safe-area-container">
        ...
        <header ref={headerRef} className="fixed top-0 left-0 right-0 z-50 top-bar-gradient border-b border-cyan-900/30 px-2 lg:px-3 py-1.5 lg:py-2" style={{ paddingTop: 'calc(0.375rem + env(safe-area-inset-top, 0px))' }}>
          ...
          {/* Mobile header - three rows for better touch targets and readability */}
          <div className="flex lg:hidden flex-col gap-0.5">
            ...
            {/* Row 2: Speed controls (larger touch targets) + power bar */}
            <div className="flex items-center gap-2">
              {/* Speed controls - larger buttons for mobile */}
            ...
            {/* Row 3: Status indicators + essential actions (less crowded) */}
            <div className="flex items-center justify-between gap-1">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  onClick={() => handleTabChange('notifications' as GameTab)}
                  role="status"
                ...
                {/* Active events badge */}
                {activeEvents.length > 0 && (
                  <Badge variant="outline" className="text-[9px] border-orange-500/50 text-orange-400 bg-orange-900/20 px-1.5 py-0 h-5 cursor-pointer" onClick={() => handleTabChange('events' as GameTab)}>
                ...
                <Badge variant="outline" className={`text-[9px] px-1.5 py-0 h-5 cursor-pointer ${
                  weather.current === 'clear'
                    ? 'border-gray-700 text-gray-500 bg-gray-900/20'
                    : 'border-sky-500/50 text-sky-400 bg-sky-900/20'
                }`} onClick={() => handleTabChange('dashboard' as GameTab)}>
              ...
              {/* Right: Auth + Save/Load actions */}
              <div className="flex items-center gap-0.5">
                {/* Auth button - larger touch target */}
                {authLoading ? (
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom" className="w-48 bg-card border-cyan-900/30 p-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <Button variant="ghost" size="sm" className="w-full justify-start h-8 text-[11px]" onClick={handleExport}><Download className="w-3 h-3 mr-1.5" /> Export Save</Button>
                        <Button variant="ghost" size="sm" className="w-full justify-start h-8 text-[11px]" onClick={handleImport}><Upload className="w-3 h-3 mr-1.5" /> Import Save</Button>
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom" className="w-48 bg-card border-cyan-900/30 p-2">
                      <div className="space-y-1">
                        <Button variant="ghost" size="sm" className="w-full justify-start h-8 text-[11px]" onClick={handleExport}><Download className="w-3 h-3 mr-1.5" /> Export Save</Button>
                        <Button variant="ghost" size="sm" className="w-full justify-start h-8 text-[11px]" onClick={handleImport}><Upload className="w-3 h-3 mr-1.5" /> Import Save</Button>
                        <Button variant="ghost" size="sm" className="w-full justify-start h-8 text-[11px] text-red-400" onClick={handleReset}><RotateCcw className="w-3 h-3 mr-1.5" /> Reset Game</Button>
                      </div>
                    </TooltipContent>
                  </Tooltip>
                )}
