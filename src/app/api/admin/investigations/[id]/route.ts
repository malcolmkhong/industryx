...
export async function GET(
  ...
  try {
    ...
    // Fetch the investigation via centralized helper
...
export async function POST(
  ...
  try {
    ...
    // Update the investigation via centralized helper
    const newStatus = action === "resolve" ? "resolved" : "dismissed";
    const updater =
      newStatus === "resolved" ? resolveInvestigation : dismissInvestigation;
    const data = await updater(investigationId, note, authResult.admin.id);
