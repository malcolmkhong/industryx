...
function rememberDefinitions(data: CachedDefinitions["data"]): CachedDefinitions {
  const cached = {
    data,
    fetchedAt: Date.now(),
  };
...
function clearDefinitionsCache(): void {
...
export async function GET() {
  try {
    ...
    rememberDefinitions(payload);

    return NextResponse.json(payload);
  } catch (error) {
    console.error("[/api/game/config/definitions] Unhandled error:", error);
