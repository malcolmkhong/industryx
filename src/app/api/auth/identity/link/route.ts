...
// The actual merge happens in /api/auth/identity/confirm-link.
//
...
export async function POST(request: NextRequest) {
  try {
    ...
    logRequestIp(request, '/api/auth/identity/link', auth.userId);

    // Phase 1: read IP + UA from request headers for the audit fields
    const { hashIp, extractClientIp } = await import('@/app/api/auth/_shared/request-ip-log-helper');
