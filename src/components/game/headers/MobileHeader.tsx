    ...
    <div className="flex lg:hidden flex-col gap-0.5">
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center bg-card rounded-lg border border-brand/20 overflow-hidden flex-shrink-0">
          ...
          >
            {paused ? <Play className="w-3.5 h-3.5 text-success" /> : <Pause className="w-3.5 h-3.5 text-warning" />}
          </Button>
        ...
        <div className="flex items-center gap-1.5 flex-1 min-w-0">
          <span className="text-[9px] text-muted-label flex-shrink-0">PWR</span>
          <div className="flex-1 h-2 bg-muted-label rounded-full overflow-hidden min-w-[40px]">
            <div
      ...
      <div className="flex items-center justify-between gap-1">
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-0.5" role="status" aria-label={showSavedFlash ? 'Game saved' : 'Save pending'}>
            ...
            <span className="text-[9px] text-muted-label">{showSavedFlash ? 'Saved' : ''}</span>
          </div>
          ...
          >
            ...
            {unreadNotifications > 0 && (
              <span className="absolute top-0.5 right-0.5 w-3.5 h-3.5 bg-danger rounded-full text-[7px] text-white flex items-center justify-center font-bold">
        ...
        <div className="flex items-center gap-0.5">
          {authLoading ? (
            <Play className="w-3.5 h-3.5 text-muted-label animate-spin" />
          ) : user ? (
            <Tooltip>
              ...
              <TooltipContent side="bottom" className="w-48 bg-card border-brand/30 p-2">
                ...
                <div className="space-y-1">
                  ...
                  <Button variant="ghost" size="sm" className="w-full justify-start h-8 text-[11px] text-danger" onClick={signOut}><LogOut className="w-3 h-3 mr-1.5" /> Sign Out</Button>
                </div>
              </TooltipContent>
            </Tooltip>
          ) : (
          ...
          {!user && (
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  className="p-1.5 rounded-md hover:bg-white/[0.04] transition-colors min-w-[32px] min-h-[32px] flex items-center justify-center text-muted-label"
              ...
              <TooltipContent side="bottom" className="w-48 bg-card border-brand/30 p-2">
                <div className="space-y-1">
                  <Button variant="ghost" size="sm" className="w-full justify-start h-8 text-[11px]" onClick={onExport}><Download className="w-3 h-3 mr-1.5" /> Export Save</Button>
                  <Button variant="ghost" size="sm" className="w-full justify-start h-8 text-[11px]" onClick={onImport}><Upload className="w-3 h-3 mr-1.5" /> Import Save</Button>
                  <Button variant="ghost" size="sm" className="w-full justify-start h-8 text-[11px] text-danger" onClick={onReset}><RotateCcw className="w-3 h-3 mr-1.5" /> Reset Game</Button>
                </div>
              </TooltipContent>
            </Tooltip>
          )}
