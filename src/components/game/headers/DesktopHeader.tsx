...
import { usePerSecondTick } from "@/lib/hooks/page/usePerSecondTick";
import {
  formatByMode,
  formatDuration,
  formatRemaining,
...
export function DesktopHeader({
  ...
  // Phase 9: drive the world-clock display from the server tick.
  // `usePerSecondTick` re-renders every 1s so the clock advances without
  // a new server push, but the anchor is still the canonical server
  // gameTick — never the client wall clock.
  const perSecondTick = usePerSecondTick();
  const displayTick = gameTick + perSecondTick;
  const worldClock = DEFAULT_WORLD_CLOCK;
  ...
  const powerPercent =
    powerGrid.totalConsumption > 0
      ? Math.min(
          100,
          (powerGrid.totalProduction / powerGrid.totalConsumption) * 100,
        )
  ...
  const incomePerMinute = useMemo(() => {
    ...
    productionSnapshot.payoutPerCycle,
    effectiveSpeed,
    payoutConfig.basePayoutInterval,
  ]);
      ...
      <div className="hidden lg:flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-4">
            ...
            >
              <div className="bg-linear-to-r from-brand/20 to-success/20 px-3 py-2 border-b border-brand/20">
                <p className="text-xs font-bold text-brand inline-flex items-center gap-1.5">
                  <GameIcon ui="money" size={14} className="inline-flex" />{" "}
                  INDUSTRIAX
                </p>
                <p className="text-[10px] text-subtle mt-0.5">
              ...
              <div className="px-3 py-2 space-y-2">
                <p className="text-[11px] text-subtle leading-relaxed">
                  ...
                  materials, research tech, and expand into megaprojects across
                  5 tiers.
                </p>
                <div className="grid grid-cols-2 gap-1.5 pt-1.5 border-t border-muted-label/20">
                  <div className="text-[10px]">
                    ...
                    <div className="text-subtle font-mono font-semibold">
                      {buildings.length}
                    </div>
                  </div>
                  <div className="text-[10px]">
                  ...
                  <div className="text-[10px]">
                    ...
                    <div className="text-premium font-mono font-semibold">
                      {prestigeState.corporationPoints} CP
                    </div>
                  </div>
                  <div className="text-[10px]">
                    <div className="text-muted-label">Player</div>
                    <div className="text-subtle font-mono font-semibold truncate">
                      {userName}
                    </div>
                  </div>
                </div>
          ...
          <div className="flex items-center gap-4 text-xs">
            <HoverCard openDelay={150} closeDelay={100}>
              <HoverCardTrigger asChild>
                <div
                  className={`stat-badge stat-badge-money bg-card rounded-lg px-3 py-1.5 border border-brand/20 cursor-default ${moneyGlow ? "money-glow" : ""}`}
                >
                  <span className="text-muted-label inline-flex items-center gap-1">
                    <GameIcon ui="money" size={14} />
                  </span>
              ...
              >
                <div className="bg-linear-to-r from-success/30/30 to-success/30/20 px-3 py-2 border-b border-brand/20">
                  <p className="text-xs font-bold text-success inline-flex items-center gap-1">
                    <GameIcon ui="money" size={14} className="inline-flex" />{" "}
                    Financial Overview
                  </p>
                </div>
                <div className="px-3 py-2 space-y-1.5">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-brand font-mono">
                      ~${formatNumber(incomePerMinute)}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-subtle">Total Earned</span>
                    <span className="text-success font-mono">
                      ${formatNumber(totalMoneyEarned)}
                    </span>
                  </div>
                </div>
              </HoverCardContent>
            ...
            <div
              className={`stat-badge stat-badge-power bg-card rounded-lg px-3 py-1.5 border border-brand/20 cursor-default ${powerGrid.overload ? "warning-pulse" : ""}`}
            >
              <span className="text-muted-label inline-flex items-center gap-1">
                <GameIcon ui="power" size={14} />
              </span>
              <span
                className={`text-sm ${powerPercent >= 80 ? "text-warning" : powerPercent >= 50 ? "text-domain" : "text-danger"}`}
              >
                {formatNumber(powerGrid.totalProduction)}MW
              </span>
              <span className="text-muted-label"> / </span>
              <span className="text-subtle text-sm">
                {formatNumber(powerGrid.totalConsumption)}MW
              </span>
              <HoverCard openDelay={150} closeDelay={100}>
                <HoverCardTrigger asChild>
                  <span
                ...
                >
                  <div
                    className={`px-3 py-2 border-b border-brand/20 ${
                      factoryEfficiency >= 0.8
                        ? "bg-success/20"
                        : factoryEfficiency >= 0.5
                          ? "bg-warning/20"
            ...
            <HoverCard openDelay={200} closeDelay={100}>
              <HoverCardTrigger asChild>
                <div className="stat-badge stat-badge-rp bg-card rounded-lg px-3 py-1.5 border border-brand/20 cursor-default">
                  <span className="text-muted-label inline-flex items-center gap-1">
                    <GameIcon ui="researchPoints" size={14} />
                  </span>
                  <span className="text-research font-mono text-sm">
              ...
              <HoverCardContent
                side="bottom"
                className="w-72 bg-card border-research/30 p-0 overflow-hidden"
              >
                ...
                <div className="px-3 py-2 space-y-1.5">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-research font-mono font-bold">
                      {formatNumber(researchPoints)} RP
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-brand font-mono">
                      {completedResearch.length} / {RESEARCH_TREE.length}
                    </span>
                  </div>
                  <p className="text-[10px] text-muted-label pt-1.5 border-t border-muted-label/20 leading-relaxed">
                    Spend RP to unlock permanent tech upgrades. RP is generated
                    by Research Labs and consumed by active research.
                  </p>
                </div>
              </HoverCardContent>
            </HoverCard>
            <HoverCard openDelay={200} closeDelay={100}>
              <HoverCardTrigger asChild>
                <div className="stat-badge stat-badge-cp bg-card rounded-lg px-3 py-1.5 border border-brand/20 cursor-default">
                  <span className="text-muted-label inline-flex items-center gap-1">
                    <GameIcon ui="corporationPoints" size={14} />
                  </span>
                  <span className="text-premium font-mono text-sm">
                    {prestigeState.corporationPoints} CP
                  </span>
                </div>
              </HoverCardTrigger>
              <HoverCardContent
                side="bottom"
                className="w-72 bg-card border-premium/30 p-0 overflow-hidden"
              >
                ...
                <div className="px-3 py-2 space-y-1.5">
                  <div className="flex justify-between text-xs">
                    <span className="text-subtle">Balance</span>
                    <span className="text-premium font-mono font-bold">
                      {prestigeState.corporationPoints} CP
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                  ...
                  <p className="text-[10px] text-muted-label pt-1.5 border-t border-muted-label/20 leading-relaxed">
                    CP is earned by completing Global Expansion. Spend it on
                    permanent bonuses like game speed, production multipliers,
                    and new building tiers.
                  </p>
                </div>
              </HoverCardContent>
            </HoverCard>
        ...
        <div className="flex items-center gap-2">
            ...
            >
              ...
              <div className="px-3 py-1.5 space-y-1">
                ...
                <p className="text-[10px] text-muted-label pt-1 border-t border-muted-label/20 leading-relaxed">
                  Each tick advances all building production, consumption, and
                  event timers. Higher speed = more ticks per second.
                </p>
              </div>
            </HoverCardContent>
          </HoverCard>
          ...
          <div
            ...
            aria-label={`World time ${formatWorldClock(displayTick, worldClock)} · tick ${displayTick}`}
          >
            <span className="text-brand font-bold" aria-hidden="true">
              {formatWorldClock(displayTick, worldClock)}
            </span>
            <span className="text-muted-label" aria-hidden="true">
              ·
            </span>
            <span aria-hidden="true">tick {displayTick}</span>
          </div>
          ...
          <HoverCard openDelay={200} closeDelay={100}>
            <HoverCardTrigger asChild>
              <Button
                variant="ghost"
                size="sm"
                className="h-7 w-7 p-0 relative focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-background"
              ...
              >
                ...
                {unreadNotifications > 0 && (
                  <span
                    className={`absolute -top-0.5 -right-0.5 h-4 rounded-full text-[11px] text-white flex items-center justify-center px-1 ${
            ...
            >
              ...
              <div className="px-3 py-1.5 space-y-1">
                <div className="flex justify-between text-[10px]">
                  ...
                  <span
                    className={`font-mono font-bold ${unreadNotifications > 0 ? "text-brand" : "text-subtle"}`}
                  >
                    {unreadNotifications}
                  </span>
                </div>
                ...
                {notifications.length === 0 ? (
                  <p className="text-[10px] text-muted-label pt-1 border-t border-muted-label/20">
                    No notifications yet. Click to view the activity log.
                  </p>
                ) : (
                  ...
                  <HoverCardTrigger asChild>
                    ...
                    >
                      <GameIcon
                        ...
                        aria-hidden="true"
                      />{" "}
                      {e.remaining <= 50 ? `${e.remaining}t` : e.name}
                    </Badge>
                  </HoverCardTrigger>
                  <HoverCardContent
                    side="bottom"
          ...
          <HoverCard openDelay={200} closeDelay={100}>
            <HoverCardTrigger asChild>
              ...
              >
                <GameIcon
                  icon={WEATHER_DEFS[weather.current]?.icon}
                  size={12}
                  className="inline-flex"
                  aria-hidden="true"
                />{" "}
                {WEATHER_DEFS[weather.current]?.name}
              </Badge>
            </HoverCardTrigger>
            <HoverCardContent
              side="bottom"
            ...
            >
              ...
              <div className="px-3 py-1.5 space-y-1">
                <p className="text-[10px] text-subtle leading-relaxed">
                  {WEATHER_DEFS[weather.current]?.description}
                </p>
                {weather.remaining > 0 && (
                  <div className="flex justify-between text-[10px] pt-1 border-t border-muted-label/20">
                    <span className="text-subtle">Remaining</span>
                ...
                <p className="text-[10px] text-muted-label pt-1 border-t border-muted-label/20 leading-relaxed">
                  Weather shifts over time and modifies production rates across
                  all buildings.
                </p>
              </div>
            </HoverCardContent>
          ...
          <HoverCard openDelay={200} closeDelay={100}>
            <HoverCardTrigger asChild>
              ...
              >
                <Check
                  aria-hidden="true"
                  className={`w-3 h-3 transition-colors duration-300 ${showSavedFlash ? "text-success" : "text-subtle"}`}
                />
                <span
                  className={showSavedFlash ? "text-success" : "text-subtle"}
                >
                  Saved
                </span>
              </div>
            ...
            <HoverCardContent
              side="bottom"
              className="w-64 bg-card border-brand/30"
            >
              ...
              <p className="text-[10px] text-subtle mt-1 leading-relaxed">
                Progress saves automatically to your browser every few seconds.
                Sign in and bind an account to enable cloud sync across devices.
              </p>
            </HoverCardContent>
          </HoverCard>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="w-48 bg-card border-brand/30"
            >
              ...
              >
                <RefreshCw className="w-3 h-3 mr-2" aria-hidden="true" /> Reload
                Config
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <HoverCard openDelay={200} closeDelay={100}>
            <HoverCardTrigger asChild>
              ...
              >
                {isUsingSupabase ? (
                  <Wifi className="w-2.5 h-2.5 mr-0.5" aria-hidden="true" />
                ) : (
                  <WifiOff className="w-2.5 h-2.5 mr-0.5" aria-hidden="true" />
                )}
            ...
            >
              <div
                className={`bg-linear-to-r px-3 py-1.5 border-b ${isUsingSupabase ? "from-success/20 to-brand/10 border-success/20" : "from-warning/20 to-domain/10 border-warning/20"}`}
              >
                <p
                  className={`text-xs font-bold inline-flex items-center gap-1.5 ${isUsingSupabase ? "text-success" : "text-warning"}`}
                >
                  {isUsingSupabase ? (
              ...
              <div className="px-3 py-1.5 space-y-1">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-6 text-[10px] mt-1 w-full text-brand hover:text-brand hover:bg-brand/10 focus-visible:ring-2 focus-visible:ring-brand"
            ...
            <HoverCard openDelay={200} closeDelay={100}>
              <HoverCardTrigger asChild>
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-7 w-7 p-0 focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                ...
                >
                  {cloudStatus === "saving" ? (
                    <Loader2
                      className="w-3.5 h-3.5 text-brand animate-spin"
                      aria-hidden="true"
                    />
                  ) : cloudStatus === "success" ? (
                    <Cloud
                    ...
                    <Cloud
                      className="w-3.5 h-3.5 text-subtle hover:text-brand transition-colors"
                      aria-hidden="true"
                    />
                  )}
                </Button>
              ...
              >
                ...
                <div className="px-3 py-1.5 space-y-1">
                  ...
                  <p className="text-[10px] text-muted-label pt-1 border-t border-muted-label/20 leading-relaxed">
                    Click to push your save to Supabase. Progress auto-saves
                    locally every few seconds.
                  </p>
                </div>
              </HoverCardContent>
            </HoverCard>
            ...
            <HoverCard openDelay={200} closeDelay={100}>
              <HoverCardTrigger asChild>
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-7 w-7 p-0 text-subtle hover:text-brand focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-background"
              ...
              <HoverCardContent
                side="bottom"
                className="w-64 bg-card border-warning/30 p-0 overflow-hidden"
              >
                ...
                <div className="px-3 py-1.5 space-y-1">
                  <p className="text-[10px] text-subtle leading-relaxed">
                    Sign in to enable cloud saves that sync across all your
                    devices.
                  </p>
                  <p className="text-[10px] text-muted-label pt-1 border-t border-muted-label/20 leading-relaxed">
                    Click the button or the &quot;Bind Account&quot; link to get
                    started.
                  </p>
                </div>
              </HoverCardContent>
          ...
          {authLoading ? (
            ...
            <HoverCard openDelay={200} closeDelay={100}>
              ...
              >
                ...
                <div className="px-3 py-1.5 space-y-1">
                  <p className="text-[10px] text-subtle leading-relaxed">
                    Link a permanent account to your progress to unlock cloud
                    save, cross-device sync, and leaderboards.
                  </p>
                  <p className="text-[10px] text-muted-label pt-1 border-t border-muted-label/20 leading-relaxed">
                    Currently playing as guest — your save stays in this browser
                    until you bind.
                  </p>
                </div>
              </HoverCardContent>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="end"
                className="w-56 bg-card border-brand/30"
              >
        ...
        >
          {topHeadlines.length > 0 ? (
            topHeadlines[displayedHeadlineIndex] && (
              <li
                key={topHeadlines[displayedHeadlineIndex].id}
            ...
            <li className="text-[11px] text-subtle italic truncate">
              Welcome to IndustriaX! Build your first Mining Drill to start
              producing resources.
            </li>
          )}
        </ul>
