...
const TIER_FILL_DARK: Record<number, string> = {
  0: "#78350f", 1: "#164e63", 2: "#7c2d12", 3: "#581c87", 4: "#064e3b", 5: "#7f1d1d",
};
const TIER_COLORS: Array<{ fill: string; stroke: string; text: string; label: string }> =
  TIER_INFO.map((info, tier) => ({
    fill: TIER_FILL_DARK[tier] ?? "#1e293b",
    stroke: info.color,
    text: info.color,
    label: tier === 0 ? "Raw" : `Tier ${tier}`,
  }));
...
export function ProductionChainPanel({
    ...
    chain.steps.forEach((step) => {
      ...
      Object.values(BUILDING_DEFS).forEach((def) => {
        if (def.outputs?.some((o) => o.resource === step)) {
          ...
          const activeCount = store.buildings.filter(
            (b) => b.type === def.type && b.active,
          ).length;
          producers.push({
            type: def.type,
            name: def.name,
  ...
  // Defensive guard: PRODUCTION_CHAINS may be empty until Supabase binds it.
  // Render happens AFTER all hooks so React rules-of-hooks are satisfied.
  if (!chain) {
    return (
      <div className="game-card rounded-xl bg-card p-4 border border-border text-center text-subtle text-sm">
        Production chains not yet loaded.
      </div>
    );
  }

  const hasBottleneck = chainBottlenecks.length > 0;
  const allProducing = chain.steps.every(
    (step) => (productionRates[step as ResourceType] ?? 0) > 0,
  );

  // SVG layout calculations
  const totalWidth =
    chain.steps.length * NODE_W +
    (chain.steps.length - 1) * GAP_X +
    PADDING * 2;
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {allProducing && chain.steps.length > 0 ? (
            <Badge
              variant="outline"
              className="text-[9px] border-success/50 text-success bg-success/20"
            >
            ...
            <Badge
              variant="outline"
              className="text-[9px] border-danger/50 text-danger bg-danger/20"
            >
      ...
      >
        ...
        >
          ...
          <defs>
            ...
            {/* Bottleneck glow */}
            <filter
              id={`bottleneck-glow-${selectedChain}`}
              x="-20%"
              y="-20%"
              width="140%"
              height="140%"
          ...
          {chain.steps.map((step, i) => {
            ...
            return (
              <g
                ...
                filter={
                  isBottleneck
                    ? `url(#bottleneck-glow-${selectedChain})`
                    : rate > 0
                      ? `url(#glow-${selectedChain})`
                      : undefined
                }
              >
                {/* Node background */}
                ...
                >
                  {formatNumber(stock)}
                  {capacity > 0 && (
                    <tspan fill="#6b7280">/{formatNumber(capacity)}</tspan>
                  )}
                </text>
                    ...
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            ...
                            style={{
                              color: isBottleneck ? "#ef4444" : tier.text,
                            }}
                          >
                            {meta.name}
                          </span>
                      ...
                      {buildings.length > 0 ? (
                        ...
                        <div className="flex items-center gap-1">
                          <span className="text-[9px] text-muted-label">
                            No buildings produce this resource
                          </span>
                        </div>
                      )}
                    </div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-muted-label">
            ...
            >
              {
                chain.steps.filter(
                  (s) => (productionRates[s as ResourceType] ?? 0) > 0,
                ).length
              }
              /{chain.steps.length}
            </span>{" "}
          ...
          {hasBottleneck && (
            <span className="text-danger">
              {chainBottlenecks.length} bottleneck
              {chainBottlenecks.length > 1 ? "s" : ""}
            </span>
          )}
        </div>
        <span className="text-muted-label">
