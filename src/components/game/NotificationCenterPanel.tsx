...
const MemoizedNotificationItem = React.memo(function MemoizedNotificationItem({
  ...
  return (
    ...
    >
      <div className={`w-6 h-6 rounded-full ${tc.bg} flex items-center justify-center text-xs ${tc.text} shrink-0`}>
      ...
      {!read && (
        <button
          ...
          className="shrink-0 w-5 h-5 rounded flex items-center justify-center text-muted-label hover:text-brand transition-colors"
          title="Mark as read"
        >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-1 overflow-x-auto">
        <Filter className="w-3.5 h-3.5 text-muted-label mr-1 shrink-0" />
        {filters.map(f => (
          <button
            key={f.id}
      ...
      {/* Notification list */}
      <div ref={listRef} className="space-y-1.5 max-h-125 overflow-y-auto game-scrollbar scroll-fade">
