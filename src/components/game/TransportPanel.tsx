...
function getTransportCost(type: TransportType): number {
  return TRANSPORT_DEFS[type].baseCost.reduce(
    (s, c) => s + (c.resource === "money" ? c.amount : 0),
    0,
  );
}
...
function getPortXY(
  ...
  switch (port.side) {
    case "top":
      return { x: w * port.fraction, y: 0 };
    case "right":
      return { x: w, y: h * port.fraction };
    case "bottom":
  ...
  relations.forEach((rel) => {
    ...
    const selectBestPort = (
      ...
      const sorted = [...ports].sort((a, b) => {
        const usageDiff =
          getUsedCount(nodeId, a.id) - getUsedCount(nodeId, b.id);
        if (usageDiff !== 0) return usageDiff;

        // For horizontal sides (top/bottom), prefer ports closer to the target's horizontal position
        if (a.side === "top" || a.side === "bottom") {
          const relFractionX = Math.max(
            0,
            Math.min(1, (relCY - (nodeCY - nodeH / 2)) / nodeH),
          );
          return (
            Math.abs(a.fraction - relFractionX) -
            Math.abs(b.fraction - relFractionX)
          );
        }
        // For vertical sides (left/right), prefer ports closer to the target's vertical position
        const relFractionY = Math.max(
          0,
          Math.min(1, (relCY - (nodeCY - nodeH / 2)) / nodeH),
        );
        return (
          Math.abs(a.fraction - relFractionY) -
          Math.abs(b.fraction - relFractionY)
        );
      });
...
function NetworkGraph({
  ...
  const nodeDims = useMemo(() => {
    ...
    nodes.forEach((n) => {
      ...
      if (isExpanded) {
        ...
        dims.set(n.id, {
          w: CFG.expandedWidth,
          h: CFG.headerHeight + rows * CFG.rowHeight + CFG.nodePadding * 2,
          expanded: true,
        });
      } else {
        dims.set(n.id, {
          w: CFG.collapsedWidth,
          h: CFG.collapsedHeight,
          expanded: false,
        });
      }
    });
  ...
  const nodePositions = useMemo(() => {
    ...
    sortedTiers.forEach(([, tierNodes], colIdx) => {
      ...
      tierNodes.forEach((n) => {
        ...
        positions.set(n.id, {
          x: CFG.canvasPadX + colIdx * CFG.columnGap,
          y: yOffset,
        });
  ...
  const svgSize = useMemo(() => {
    ...
    nodePositions.forEach((pos, id) => {
      ...
      if (pos.x + (d?.w ?? CFG.collapsedWidth) > maxX)
        maxX = pos.x + (d?.w ?? CFG.collapsedWidth);
      if (pos.y + (d?.h ?? CFG.collapsedHeight) > maxY)
        maxY = pos.y + (d?.h ?? CFG.collapsedHeight);
    });
    return {
      width: Math.max(400, maxX + CFG.canvasPadX),
      height: Math.max(250, maxY + CFG.canvasPadY),
    };
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (
        Math.abs(e.clientX - panStartRef.current.mx) > 3 ||
        Math.abs(e.clientY - panStartRef.current.my) > 3
      ) {
      ...
      <div className="absolute top-2 left-2 z-10 flex items-center gap-2">
        {Array.from(tierColumns.values()).map((col) => (
          <div key={col.label} className="flex items-center gap-1">
            ...
            />
            <span className="text-[11px] text-muted-label font-medium">
              {col.label}
            </span>
          </div>
        ))}
      ...
      >
        ...
        >
          ...
          <defs>
            ...
            >
              <path
                d={`M ${CFG.columnGap} 0 L 0 0 0 40`}
                fill="none"
                stroke="#111827"
                strokeWidth={0.5}
              />
          ...
          <g className="relations-layer">
            {relations.map((rel) => {
              ...
              const isConnected =
                focusedNodeId === rel.fromId || focusedNodeId === rel.toId;
              ...
              const resEmojis = rel.resources
                .slice(0, 2)
                .map((r) => RESOURCE_META[r.resource]?.icon ?? "")
                .join("");
              const moreN =
                rel.resources.length > 2 ? rel.resources.length - 2 : 0;
                ...
                >
                  ...
                  <g>
                    ...
                    >
                      {resEmojis}
                      {moreN > 0 ? `+${moreN}` : ""}{" "}
                      {formatNumber(rel.totalThroughput)}/s
                    </text>
                  </g>
                  {/* Line count badge */}
                  {rel.lineCount > 1 && (
                    ...
                    {isFocused && (
                      <>
                        <rect
                          ...
                          opacity={0.2}
                          style={{
                            filter: `drop-shadow(0 0 6px ${tc.stroke})`,
                          }}
                        />
                        <rect
                          x={-2}
                    ...
                    {isExpanded ? (
                      ...
                      <g style={{ pointerEvents: "none" }}>
                        ...
                        {(() => {
                            ...
                            rows.push({
                              ...
                              value:
                                node.outputs
                                  ...
                                  .map(
                                    (r) =>
                                      RESOURCE_META[r as ResourceType]?.icon ??
                                      r,
                                  )
                                  .join(" ") +
                          ...
                          rows.push({
                            label: `Lv${node.level}`,
                            value: `${statusIcon} ${node.active ? "ON" : "OFF"}`,
...
export function TransportPanel() {
  const store = useGameStore(
    useShallow((s) => ({
      buildTransportLine: s.buildTransportLine,
      buildings: s.buildings,
      money: s.money,
      powerGrid: s.powerGrid,
      productionSnapshot: s.productionSnapshot,
      toggleTransportLine: s.toggleTransportLine,
      transportLines: s.transportLines,
      upgradeTransportLine: s.upgradeTransportLine,
      weather: s.weather,
    })),
  );
  const [selectedType, setSelectedType] =
    useState<TransportType>("conveyorBelt");
  ...
  >({ lines: true, throughput: true });

  // --- Derived Data ---
  const activeLines = useMemo(
    () => store.transportLines.filter((l) => l.active),
    [store.transportLines],
  );
  const totalThroughput = useMemo(
    () => activeLines.reduce((sum, l) => sum + l.throughput, 0),
    [activeLines],
  );
  const totalMaxThroughput = useMemo(
    () => store.transportLines.reduce((sum, l) => sum + l.maxThroughput, 0),
    [store.transportLines],
  );
      ...
      if (val) {
        ...
        const outputs =
          def?.outputs
            ...
            .map((o) => o.resource as ResourceType) ?? [];
        if (outputs.length > 0) {
          setCarriesResource((prev) =>
            outputs.includes(prev as ResourceType) ? prev : outputs[0],
          );
        } else {
          setCarriesResource("");
        }
  ...
  const previewData = useMemo(() => {
    ...
    const outputAmount =
      fromDef?.outputs?.find((o) => o.resource === carriesResource)?.amount ??
      0;
    const inputAmount =
      toDef?.inputs?.find((i) => i.resource === carriesResource)?.amount ?? 0;
    const estimatedThroughput = Math.min(
      def.baseThroughput,
      outputAmount * fromB.level,
      inputAmount * toB.level,
    );
  ...
  const routeSuggestions = useMemo(() => {
    ...
    const existingRoutes = new Set(
      store.transportLines.map(
        (l) => `${l.fromBuilding}-${l.toBuilding}-${l.carriesResource}`,
      ),
    );
  ...
  const handleCreateSuggestedRoute = useCallback(
    (from: string, to: string, resource: ResourceType) => {
  ...
  const handleBuild = useCallback(() => {
    ...
    store.buildTransportLine(
      selectedType,
      fromBuilding,
      toBuilding,
      carriesResource as ResourceType,
    );
    setFromBuilding("");
    setToBuilding("");
    setCarriesResource("");
  ...
  const productionChain = useMemo(() => {
    ...
    for (let t = 0; t <= 4; t++) {
      const tierBuildings = store.buildings.filter(
        (b) => getBuildingTier(b) === t,
      );
      if (tierBuildings.length === 0) continue;
      const items = tierBuildings.map((b) => {
        const def = BUILDING_DEFS[b.type];
        const hasOut = store.transportLines.some(
          (l) => l.fromBuilding === b.id,
        );
      ...
      tiers.push({
        tier: t,
        label: TIER_COLORS[t]?.label ?? `T${t}`,
        buildings: items,
      });
    }
    const totalBuildings = tiers.reduce((s, t) => s + t.buildings.length, 0);
    const connectedBuildings = tiers.reduce(
      (s, t) => s + t.buildings.filter((b) => b.connected).length,
      0,
    );
    const completeness =
      totalBuildings > 0 ? (connectedBuildings / totalBuildings) * 100 : 100;
  ...
  const networkHealth = useMemo(() => {
    ...
    const buildingsWithTransport = store.buildings.filter((b) =>
      store.transportLines.some(
        (l) => l.fromBuilding === b.id || l.toBuilding === b.id,
      ),
    ).length;
    const connectivity =
      store.buildings.length > 0
        ? (buildingsWithTransport / store.buildings.length) * 100
        : 100;
    const activeRate =
    ...
    const avgUtilization =
      store.transportLines.length > 0
        ? (store.transportLines.reduce(
            (s, l) => s + l.throughput / l.maxThroughput,
            0,
          ) /
            store.transportLines.length) *
          100
        : 100;
    ...
    store.buildings.forEach((b) => {
      ...
      if (def.outputs && def.outputs.length > 0) {
        const outLines = store.transportLines.filter(
          (l) => l.fromBuilding === b.id && l.active,
        );
        if (
          outLines.length === 0 &&
          producingBuildings.some((pb) => pb.id === b.id)
        )
          bottleneckCount++;
      }
      if (def.inputs && def.inputs.length > 0) {
        def.inputs.forEach((input) => {
          if (input.resource === "money") return;
          const inLines = store.transportLines.filter(
            (l) =>
              l.toBuilding === b.id &&
              l.carriesResource === (input.resource as ResourceType) &&
              l.active,
          );
      ...
      store.transportLines
        .filter((l) => l.fromBuilding === b.id && l.active)
        .forEach((line) => {
  ...
  const bottlenecks = useMemo(() => {
    ...
    store.buildings.forEach((b) => {
      ...
      if (def.outputs && def.outputs.length > 0) {
        ...
        if (
          ...
          const outputResources = def.outputs.map(
            (o) => o.resource as ResourceType,
          );
          ...
          const totalOutput = buildingSnapshot.outputs.reduce(
            (sum, o) => sum + o.amount,
            0,
          );
          const matchingConsumers = consumingBuildings.filter((cb) => {
            const cbDef = BUILDING_DEFS[cb.type];
            return cbDef?.inputs?.some((i) =>
              outputResources.includes(i.resource as ResourceType),
            );
          });
          issues.push({
            building: b,
            ...
            action:
              matchingConsumers.length > 0
                ? {
                    label: "Create Route",
                    onClick: () =>
                      handleCreateSuggestedRoute(
                        b.id,
                        matchingConsumers[0].id,
                        outputResources[0],
                      ),
                  }
                : undefined,
      ...
      if (def.inputs && def.inputs.length > 0) {
        ...
        def.inputs.forEach((input) => {
          ...
          const inLines = store.transportLines.filter(
            (l) =>
              l.toBuilding === b.id && l.carriesResource === res && l.active,
          );
          const totalInboundRate = inLines.reduce(
            (s, l) => s + l.throughput,
            0,
          );
          // Use snapshot input rate (includes all multipliers: efficiency, research, events)
          const consumptionRate =
            buildingSnapshot.inputs.find((i) => i.resource === res)?.amount ??
            input.amount * b.level;
              ...
              action:
                producers.length > 0
                  ? {
                      label: "Create Route",
                      onClick: () =>
                        handleCreateSuggestedRoute(producers[0].id, b.id, res),
                    }
                  : undefined,
            });
      ...
      if (def.outputs && def.outputs.length > 0) {
        ...
        def.outputs.forEach((output) => {
          ...
          const productionRate =
            buildingSnapshot.outputs.find((o) => o.resource === res)?.amount ??
            output.amount * b.level;
          const outLines = store.transportLines.filter(
            (l) =>
              l.fromBuilding === b.id && l.carriesResource === res && l.active,
          );
          const totalOutboundCapacity = outLines.reduce(
            (s, l) => s + l.maxThroughput,
            0,
          );
          const totalOutboundThroughput = outLines.reduce(
            (s, l) => s + l.throughput,
            0,
          );
          ...
          if (
            outLines.length > 0 &&
            totalOutboundCapacity > productionRate * 2 &&
      ...
      if (b.efficiency < 0.5 && store.powerGrid.overload) {
        issues.push({
          ...
          type: "power",
          solution:
            "Build more power plants or deactivate non-essential buildings.",
        });
      }
    });
    ...
    return issues;
    // Per-field deps (store.buildings etc.) keep memo from re-running on
    // unrelated tick updates. Whole `store` is intentionally excluded
    // because it changes on every tick and would defeat the memo.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    store.buildings,
    store.transportLines,
    store.powerGrid.overload,
    store.productionSnapshot.buildings,
    producingBuildings,
    consumingBuildings,
  ...
  const resourceFlow = useMemo(() => {
    ...
    producingBuildings.forEach((b) => {
      ...
      buildingSnapshot.outputs.forEach((o) => {
        ...
        const existing = flowMap.get(res) ?? {
          production: 0,
          consumption: 0,
          surplus: 0,
          name: RESOURCE_META[res]?.name ?? res,
          icon: RESOURCE_META[res]?.icon ?? "",
          tier: RESOURCE_META[res]?.tier ?? 0,
          color: RESOURCE_META[res]?.color ?? "#888",
        };
    ...
    flowMap.forEach((v) => {
      v.surplus = v.production - v.consumption;
    });
  ...
  const throughputByType = useMemo(() => {
    return TRANSPORT_TYPES.map((type) => {
        ...
        return {
          ...
          inputs:
            def.inputs
              ...
              .map((i) => i.resource as string) ?? [],
          outputs:
            def.outputs
              ?.filter((o) => o.resource !== "money")
              .map((o) => o.resource as string) ?? [],
          active: b.active,
          level: b.level,
        } as ERDNode;
      })
  ...
  // --- Bulk Operations ---
  const handleUpgradeAllType = useCallback(
    (type: TransportType) => {
      store.transportLines
  ...
  const handleActivateAll = useCallback(() => {
    store.transportLines
      .filter((l) => !l.active)
      .forEach((l) => store.toggleTransportLine(l.id));
  }, [store]);
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-muted-label mt-0.5">
            Manage supply chains and logistics networks
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Badge
            variant="outline"
            className="border-warning/80/50 text-warning bg-warning/10 text-[10px]"
          >
            <GameIcon
          ...
          <Badge
            variant="outline"
            className="border-brand/50 text-brand bg-brand/10 text-xs"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-3">
        ...
        <div className="game-card rounded-xl bg-card p-3 border border-brand/30 col-span-2 sm:col-span-1 flex flex-col items-center justify-center relative overflow-hidden">
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse at 50% 50%, #22d3ee, transparent 70%)",
            }}
          />
          <div className="relative z-10 flex flex-col items-center">
            <div className="text-[10px] text-muted-label mb-1">
              Network Health
            </div>
            ...
            <div
              className={`text-[10px] font-mono font-bold ${healthTextColor}`}
            >
              {networkHealth.score.toFixed(0)}%
            </div>
          </div>
        ...
        {store.transportLines.length === 0 ? (
          <div className="text-center py-8 border border-dashed border-muted-label rounded-lg">
            ...
            <p className="text-[10px] text-muted-label mt-1">
              Build transport lines to see the auto-generated network
            </p>
          </div>
        ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-background rounded-lg p-3 border border-warning/20">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div>
                ...
                <div className="text-[10px] text-muted-label">
                  Intensity: {(store.weather.intensity * 100).toFixed(0)}%
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div>
                ...
                <div
                  className={`text-[11px] font-mono font-bold ${weatherDef.productionMultiplier >= 1 ? "text-success" : weatherDef.productionMultiplier > 0.85 ? "text-warning" : "text-danger"}`}
                >
              ...
              <div>
                ...
                <div
                  className={`text-[11px] font-mono font-bold ${weatherDef.solarMultiplier >= 1 ? "text-success" : weatherDef.solarMultiplier > 0.5 ? "text-warning" : "text-danger"}`}
                >
              ...
              <div>
                ...
                <div
                  className={`text-[11px] font-mono font-bold ${weatherDef.windMultiplier >= 1 ? "text-success" : weatherDef.windMultiplier > 0.5 ? "text-warning" : "text-danger"}`}
                >
        ...
        <div className="game-card rounded-xl bg-card p-4 border border-border">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] text-muted-label ml-auto">
              {productionChain.completeness.toFixed(0)}% connected
            </span>
          </div>
          {/* Completeness bar */}
          <div className="h-2 bg-muted-label rounded-full overflow-hidden mb-3">
            <div
              ...
              style={{
                width: `${Math.min(100, productionChain.completeness)}%`,
              }}
            />
          </div>
          <div className="space-y-2 max-h-48 overflow-y-auto game-scrollbar">
            {productionChain.tiers.map((tier) => {
              ...
              return (
                <div
                  key={tier.tier}
                  className="bg-background rounded-lg p-2 border border-muted-label/50"
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    <Badge
                      variant="outline"
                      className={`text-[9px] px-1.5 ${tc.text} border-current`}
                    >
                      {tc.label}
                    </Badge>
                    <span className="text-[10px] text-subtle">
                      {tier.buildings.filter((b) => b.connected).length}/
                      {tier.buildings.length} connected
                    </span>
                  </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="game-card rounded-xl bg-card p-4 border border-border">
                ...
                return (
                  <GameItemTooltip
                    ...
                    details={[
                      {
                        label: "Throughput",
                        value: `${def.baseThroughput.toFixed(1)} u/s`,
                        color: "text-brand",
                      },
                      {
                        label: "Base Cost",
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  <label
                    htmlFor="transport-from"
                    className="text-[10px] text-muted-label mb-1 block"
                  >
                    From (Producer)
                  </label>
                  <select
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setCarriesResource(e.target.value as ResourceType);
                      setToBuilding("");
                    }}
                    className="w-full bg-background border border-muted-label rounded-lg px-2 py-1.5 text-xs text-subtle focus:border-brand/50 focus:outline-none"
              ...
              {/* Live Preview */}
              {previewData && (
                <div className="bg-background rounded-lg p-3 border border-brand/30">
                  <div className="text-[10px] text-brand font-semibold mb-2">
                    ROUTE PREVIEW
                  </div>
                  <div className="flex items-center gap-2 mb-2 text-xs">
                    <GameIcon icon={previewData.fromDef?.icon} size={16} />
                    <span className="text-subtle">
                      {previewData.fromDef?.name}
                    </span>
                    <ArrowRight className="w-3 h-3 text-brand" />
                    <GameIcon
                      icon={
                        RESOURCE_META[carriesResource as ResourceType]?.icon
                      }
                      size={16}
                    />
                    <span className="text-subtle">
                      {RESOURCE_META[carriesResource as ResourceType]?.name}
                    </span>
                    <ArrowRight className="w-3 h-3 text-brand" />
                    <GameIcon icon={previewData.toDef?.icon} size={16} />
                    <span className="text-subtle">
                      {previewData.toDef?.name}
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center text-[10px]">
                    <div>
                      ...
                      <div className="text-brand font-mono font-bold">
                        {previewData.estimatedThroughput.toFixed(1)} u/s
                      </div>
                    </div>
                    <div>
                      <span className="text-muted-label">Cost</span>
                      <div
                        className={`font-mono font-bold ${previewData.canAfford ? "text-success" : "text-danger"}`}
                      >
                        ${formatNumber(previewData.cost)}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="text-brand font-mono font-bold">
                        <GameIcon
                          icon={TRANSPORT_DEFS[selectedType].icon}
                          size={14}
                          className="inline-flex"
                        />{" "}
                        {TRANSPORT_DEFS[selectedType].name}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="game-card rounded-xl bg-card p-4 border border-border">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] text-muted-label ml-auto">
                {store.transportLines.length} total
              </span>
              <button
                onClick={() => toggleSection("lines")}
                className="text-muted-label hover:text-subtle"
              >
                ...
                <div>
                  {store.transportLines.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-xs text-muted-label">
                        No transport lines built yet
                      </p>
                      <p className="text-[10px] text-muted-label mt-1">
                        Connect producers to consumers to optimize production
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-2 max-h-96 overflow-y-auto game-scrollbar">
                      {store.transportLines.map((line) => {
                        const def = TRANSPORT_DEFS[line.type];
                        const fromB = store.buildings.find(
                          (b) => b.id === line.fromBuilding,
                        );
                        const toB = store.buildings.find(
                          (b) => b.id === line.toBuilding,
                        );
                        const fromDef = fromB
                          ? BUILDING_DEFS[fromB.type]
                          : null;
                        const toDef = toB ? BUILDING_DEFS[toB.type] : null;
                        const upgradeCost = getUpgradeCost(line);
                        const throughputPct =
                          line.maxThroughput > 0
                            ? (line.throughput / line.maxThroughput) * 100
                            : 0;

                        return (
                          <div
                            key={line.id}
                            className={`bg-background rounded-lg p-3 border ${line.active ? "border-brand/30 hover:border-brand/50" : "border-muted-label opacity-60"}`}
                          >
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-1.5 flex-1 min-w-0">
                                <div className="flex items-center gap-1 bg-muted-label/50 rounded px-1.5 py-0.5 min-w-0">
                                  ...
                                  <span className="text-[10px] text-subtle truncate max-w-17.5">
                                    {fromDef?.name}
                                  </span>
                                </div>
                                <div className="flex items-center gap-0.5 shrink-0">
                                  ...
                                  <GameIcon
                                    icon={
                                      RESOURCE_META[line.carriesResource]?.icon
                                    }
                                    size={12}
                                  />
                                  <ArrowRight className="w-3 h-3 text-brand" />
                            ...
                            <div className="h-2 bg-muted-label rounded-full overflow-hidden relative mb-2">
                              <div
                                className={`h-full rounded-full transition-all duration-700 ${
                                  throughputPct > 80
                                    ? "bg-linear-to-r from-danger/60 to-danger/60"
                                    : throughputPct > 50
                                      ? "bg-linear-to-r from-warning/80 to-warning/50"
                                      : "bg-linear-to-r from-brand/80 to-brand/50"
                                }`}
                                style={{
                                  width: `${Math.min(100, throughputPct)}%`,
                                }}
                              />
                            </div>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                <Badge
                                  variant="outline"
                                  className="text-[9px] border-muted-label text-subtle px-1"
                                >
                                  Lv.{line.level}
                                </Badge>
                                <span className="text-[9px] text-muted-label font-mono">
                                  {line.throughput.toFixed(1)}/
                                  {line.maxThroughput.toFixed(1)}
                                </span>
                                <button
                                  onClick={() =>
                                    store.toggleTransportLine(line.id)
                                  }
                                  className={`w-5 h-5 rounded-full border text-[10px] flex items-center justify-center ${
                                    line.active
                                      ? "border-success/50 text-success bg-success/20 hover:bg-success/40"
                                      : "border-muted-label text-muted-label hover:bg-muted-label"
                                  }`}
                                  title={
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  store.upgradeTransportLine(line.id)
                                }
            ...
            <div className="game-card rounded-xl bg-card p-4 border border-border">
              <div className="flex items-center gap-2 mb-3">
                ...
                <button
                  ...
                  className="text-muted-label hover:text-subtle ml-auto"
                >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="pt-2 border-t border-muted-label flex items-center justify-between">
                        ...
                        <span className="text-xs text-brand font-mono font-bold">
                          ...
                          {formatNumber(totalMaxThroughput)} u/s
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="space-y-4">
              ...
              <div className="space-y-1 max-h-56 overflow-y-auto game-scrollbar">
                ...
                {resourceFlow.map((r) => {
                  ...
                  return (
                    <div
                      key={r.resource}
                      className="grid grid-cols-[auto_1fr_auto_auto_auto] gap-x-2 text-[10px] items-center px-1 py-0.5 hover:bg-muted-label/30 rounded"
                    >
                      <GameIcon icon={r.icon} size={16} />
                      <span className="text-subtle truncate">{r.name}</span>
                      <span className="text-success font-mono text-right">
                        {r.production.toFixed(1)}
                      </span>
                      <span className="text-danger font-mono text-right">
          ...
          <div className="game-card rounded-xl bg-card p-4 border border-domain/30">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-semibold text-domain">
                Bottleneck Detection
              </h3>
              {bottlenecks.length > 0 && (
                <Badge
                  variant="outline"
                  className="text-[9px] border-danger/50 text-danger ml-auto"
                >
                  {bottlenecks.length} issue
            ...
            {bottlenecks.length === 0 ? (
              <div className="text-center py-4">
                ...
                <p className="text-[9px] text-muted-label mt-1">
                  All transport lines operating normally
                </p>
              </div>
            ) : (
                ...
                {bottlenecks.map((bn, i) => {
                  ...
                  const sevColor =
                    bn.severity === "critical"
                      ? "text-danger border-danger/40/30"
                      : bn.severity === "warning"
                        ? "text-warning border-warning/30"
                        : "text-subtle border-muted-label";
                  const sevIcon =
                    bn.severity === "critical" ? (
                      ...
                      <CircleDot className="w-3 h-3 text-subtle" />
                    );
                  const typeIcon =
                    bn.type === "under-supplied" ? (
                      <TrendingDown className="w-3 h-3 text-danger" />
                    ) : bn.type === "over-supplied" ? (
                      <TrendingUp className="w-3 h-3 text-warning" />
                    ) : bn.type === "capacity" ? (
                      <Zap className="w-3 h-3 text-domain" />
                    ) : bn.type === "power" ? (
                      <ZapOff className="w-3 h-3 text-warning" />
                    ) : (
                      <Route className="w-3 h-3 text-danger" />
                    );
                  return (
                    <div
                      key={i}
                    ...
                    >
                      ...
                      {bn.flowRate !== undefined &&
                        bn.requiredRate !== undefined && (
                          <div className="text-[9px] text-muted-label mb-1">
                            Flow:{" "}
                            <span className="font-mono">
                              {bn.flowRate.toFixed(1)}
                            </span>{" "}
                            / Required:{" "}
                            <span className="font-mono">
                              {bn.requiredRate.toFixed(1)}
                            </span>{" "}
                            /s
                          </div>
                        )}
                      <p className="text-[9px] text-muted-label">
                        {bn.solution}
                      </p>
                      {bn.action && (
                        <Button
            ...
            <div className="space-y-2">
              ...
              <div className="border-t border-muted-label pt-2">
                ...
                >
                  ...
                  Connect All ({connectAllData.routes.length} routes — $
          ...
          <div className="game-card rounded-xl bg-card p-4 border border-border">
            ...
            <div className="space-y-2">
              {[
                {
                  label: "Connectivity",
                  value: networkHealth.details.connectivity,
                  icon: <Network className="w-3 h-3" />,
                },
                {
                  label: "Active Rate",
                  value: networkHealth.details.activeRate,
                  icon: <Play className="w-3 h-3" />,
                },
                {
                  label: "Utilization",
                  value: networkHealth.details.utilization,
                  icon: <BarChart3 className="w-3 h-3" />,
                },
                {
                  label: "Bottleneck Penalty",
                  value: -networkHealth.details.bottleneckPenalty,
            ...
            <div className="game-card rounded-xl bg-card p-4 border border-brand/30">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className="text-[9px] border-brand/50 text-brand"
                  >
                    {routeSuggestions.length}
                  </Badge>
                </div>
              ...
              {routeSuggestions.length === 0 ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-[10px] text-muted-label mt-1">
                    Build more producers and consumers to get route suggestions
                  </p>
                </div>
              ) : (
                <div className="space-y-2 max-h-64 overflow-y-auto game-scrollbar">
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2">
                          <GameIcon
                            ...
                            className="inline-flex"
                          />
                          <span className="text-xs text-subtle truncate max-w-20">
                            {fromDef?.name}
                          </span>
            ...
            >
              ...
              <p className="text-xs text-subtle mb-3">
                ...
                <span className="text-brand font-bold">
                  {connectAllData.routes.length}
                </span>{" "}
                transport lines using{" "}
                <GameIcon
                  icon={TRANSPORT_DEFS[CHEAPEST_TYPE].icon}
                  size={14}
                  className="inline-flex"
                />{" "}
                {TRANSPORT_DEFS[CHEAPEST_TYPE].name} (cheapest).
              </p>
              <div className="bg-background rounded-lg p-3 mb-4 max-h-40 overflow-y-auto game-scrollbar">
                ...
                {connectAllData.routes.length > 20 && (
                  <div className="text-[10px] text-muted-label mt-1">
                    ...and {connectAllData.routes.length - 20} more
                  </div>
                )}
              </div>
              <div className="flex items-center justify-between mb-4">
                ...
                <span
                  className={`text-sm font-bold font-mono ${connectAllData.canAfford ? "text-success" : "text-danger"}`}
                >
                  ${formatNumber(connectAllData.totalCost)}
                </span>
              </div>
              <div className="flex gap-3">
                ...
                <Button
                  ...
                  disabled={
                    !connectAllData.canAfford ||
                    connectAllData.routes.length === 0
                  }
                >
