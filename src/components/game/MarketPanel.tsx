...
function BezierSparkline({
  ...
  const baseLineY =
    baseValue != null
      ? padding + chartH - ((baseValue - min) / range) * chartH
      : 0;
    ...
    <div className="flex items-center gap-1">
      <div className="flex-1 flex items-center gap-0.5">
        <div className="flex-1 h-1.5 bg-muted-label rounded-full overflow-hidden flex">
          <div
            className="h-full bg-domain/60 rounded-l-full"
            style={{ width: `${demandPct}%` }}
          />
        </div>
        <div className="flex-1 h-1.5 bg-muted-label rounded-full overflow-hidden flex">
          <div
            className="h-full bg-brand/60 rounded-l-full"
            style={{ width: `${supplyPct}%` }}
          />
        </div>
      </div>
      <span
        className={`text-[11px] font-mono font-bold ${net > 0.1 ? "text-domain" : net < -0.1 ? "text-brand" : "text-muted-label"}`}
      >
        {net > 0.1 ? "▲" : net < -0.1 ? "▼" : "─"}
      </span>
    </div>
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
      {sectors.map((sector) => {
        ...
        const sectorRes = market.filter(
          (m) => RESOURCE_SECTOR[m.resource] === sector,
        );
        const avgRatio =
          sectorRes.length > 0
            ? sectorRes.reduce((s, m) => s + m.currentPrice / m.basePrice, 0) /
              sectorRes.length
            : 1;
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-0.5">
              <GameIcon
                icon={info.icon as `game-icons:${string}` | `lucide:${string}`}
                size={12}
              ...
              {trend === "down" && (
                <span className="text-[11px] text-danger">▼</span>
              )}
              {trend === "stable" && (
                <span className="text-[11px] text-muted-label">─</span>
              )}
            </div>
            <div className={`text-[9px] font-medium ${info.color}`}>
              {info.name}
            </div>
            <div className="text-[9px] text-muted-label font-mono">
...
export function MarketPanel() {
  const store = useGameStore(
    useShallow((s) => ({
      ...
      sectorTrends: s.sectorTrends,
      sellResource: s.sellResource,
      serverMarket: s.serverMarket,
      toggleAutoSell: s.toggleAutoSell,
    })),
  );
  ...
  const [viewMode, setViewMode] = useState<
    "market" | "sectors" | "chains" | "news"
  >("market");
  const [newsFilter, setNewsFilter] = useState<
    "all" | "price_move" | "volatility" | "sector" | "trade"
  >("all");
  ...
  const portfolioValue = useMemo(() => {
    return (Object.entries(store.resources) as [ResourceType, number][]).reduce(
      (sum, [resource, amount]) => {
        const price =
          store.market.find((m) => m.resource === resource)?.currentPrice ?? 0;
  ...
  const marketSummary = useMemo(() => {
    const prices = store.market.map((m) => ({
      resource: m.resource,
      ratio: m.currentPrice / m.basePrice,
      trend: m.trend,
    }));
    const avgRatio =
      prices.reduce((sum, p) => sum + p.ratio, 0) / prices.length;
    const sentiment =
      avgRatio > 1.15 ? "Bullish" : avgRatio < 0.85 ? "Bearish" : "Neutral";
    const sentimentDot =
      avgRatio > 1.15
        ? "bg-success"
        : avgRatio < 0.85
  ...
  const selected = selectedResource
    ? store.market.find((m) => m.resource === selectedResource)
    : null;
  const selectedMeta = selectedResource
    ? RESOURCE_META[selectedResource]
    : null;
  ...
  const correlationChain = useMemo(() => {
    ...
    const upstream = PRICE_CORRELATIONS.filter(
      (c) => c.to === selectedResource,
    ).map((c) => ({
      resource: c.from,
      strength: c.strength,
      direction: "upstream" as const,
    }));
    const downstream = PRICE_CORRELATIONS.filter(
      (c) => c.from === selectedResource,
    ).map((c) => ({
      resource: c.to,
      strength: c.strength,
  ...
  const playerImpact = useMemo(() => {
    if (!selectedResource)
      return { production: 0, consumption: 0, netPressure: 0 };
    const prod = store.productionSnapshot?.production[selectedResource] ?? 0;
    const cons =
      store.productionSnapshot?.actualConsumption[selectedResource] ?? 0;
  ...
  const maxSell = selectedResource
    ? Math.floor(store.resources[selectedResource])
    : 0;
  const maxBuy = useMemo(() => {
    ...
    const byCapacity = Math.floor(
      store.resourceCapacity[selectedResource] -
        store.resources[selectedResource],
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="bg-linear-to-r from-success/30/10 to-transparent m-0 p-0 rounded-xl">
          ...
          <p className="text-xs text-muted-label mt-0.5">
            Supply & demand driven economy — your production shapes prices
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Badge
            variant="outline"
            className="border-success/50 text-success bg-success/20 text-xs"
          >
            <Wallet className="w-3 h-3 mr-1" />${formatNumber(store.money)}
          </Badge>
          <Badge
            variant="outline"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <PanelStatCard
          ...
          value={
            <>
              <span
                className={`inline-block w-2.5 h-2.5 rounded-full ${marketSummary.sentimentDot}`}
              />{" "}
          ...
          color={
            marketSummary.sentiment === "Bullish"
              ? "green"
              : marketSummary.sentiment === "Bearish"
                ? "red"
                : "yellow"
          }
          trend={
        ...
        <PanelStatCard
          ...
          value={
            <>
              <GameIcon
                icon={marketSummary.bestSellMeta?.icon}
                size={14}
                className="inline-flex"
              />{" "}
              {(marketSummary.bestSell?.ratio * 100).toFixed(0)}%
            </>
          }
      ...
      {viewMode === "sectors" && (
        <SectorOverview
          sectorTrends={store.sectorTrends ?? {}}
          market={store.market}
        />
      )}
      ...
      {viewMode === "chains" && (
        <div className="game-card rounded-xl bg-card p-4 border border-border">
          ...
          <div className="text-[10px] text-muted-label mb-3">
            When an upstream resource price changes, it ripples through to
            downstream resources. Select a resource to see its chain.
          </div>
          {selectedResource ? (
            <div className="space-y-3">
              ...
              {correlationChain.upstream.length > 0 && (
                <div>
                  <div className="text-[10px] text-domain font-semibold mb-1.5">
                    ↑ Upstream (affects this price)
                  </div>
                  <div className="space-y-1">
                    {correlationChain.upstream.map((c) => {
                      const meta = RESOURCE_META[c.resource];
                      const m = store.market.find(
                        (x) => x.resource === c.resource,
                      );
                      return (
                        <button
                          key={c.resource}
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] text-domain font-mono">
              ...
              {correlationChain.downstream.length > 0 && (
                <div>
                  <div className="text-[10px] text-success font-semibold mb-1.5">
                    ↓ Downstream (this price affects)
                  </div>
                  <div className="space-y-1">
              ...
              {correlationChain.upstream.length === 0 &&
                correlationChain.downstream.length === 0 && (
                  <div className="text-center py-4 text-[10px] text-muted-label">
                    No correlations defined for this resource
                  </div>
                )}
            </div>
          ) : (
            <div className="text-center py-6 text-[10px] text-muted-label">
              Select a resource from the market list to see its price
              correlation chain
            </div>
          )}
        </div>
          ...
          <div className="lg:col-span-2 space-y-4">
              ...
              {(
                ...
                return (
                  ...
                  >
                    <GameIcon
                      icon={
                        info.icon as `game-icons:${string}` | `lucide:${string}`
                      }
                      size={10}
                    />
                    {info.name.split(" ")[0]}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-125 overflow-y-auto game-scrollbar scroll-fade">
              ...
              {filteredMarket.map((m) => {
                ...
                const prod =
                  store.productionSnapshot?.production[m.resource] ?? 0;
                const cons =
                  store.productionSnapshot?.actualConsumption[m.resource] ?? 0;
                const activeInjection = null as {
                  source: string;
                  intensity: number;
                } | null;
                ...
                return (
                  <GameItemTooltip
                    ...
                    details={[
                      {
                        label: "Current Price",
                        value: `$${m.currentPrice.toFixed(2)}`,
                        color:
                          m.trend === "up"
                            ? "text-success"
                            : m.trend === "down"
                      ...
                      {
                        label: "Demand",
                        value: `${m.demand.toFixed(2)}x`,
                        color: "text-domain",
                      },
                      {
                        label: "Supply",
                        value: `${m.supply.toFixed(2)}x`,
                        color: "text-brand",
                      },
                      {
                        label: "Elasticity",
                        value: `${(elasticity * 100).toFixed(0)}%`,
                        color: elasticity > 0.5 ? "text-danger" : "text-subtle",
                      },
                      {
                        label: "Volatility",
                        value: activeInjection
                          ? `${activeInjection.source} (${activeInjection.intensity.toFixed(2)})`
                          : "None",
                        color: activeInjection
                          ? "text-warning"
                          : "text-muted-label",
                      },
                      {
                        label: "Your Production",
                        value: prod > 0 ? `${prod.toFixed(1)}/s` : "—",
                        color: prod > 0 ? "text-success" : "text-muted-label",
                      },
                      {
                        label: "Your Consumption",
                        value: cons > 0 ? `${cons.toFixed(1)}/s` : "—",
                        color: cons > 0 ? "text-domain" : "text-muted-label",
                      },
                      {
                        label: "Auto-Sell",
                        value: isAutoSell ? "Enabled" : "Disabled",
                        color: isAutoSell ? "text-success" : "text-muted-label",
                      },
                    ]}
                    side="right"
                  >
                    <button
                      onClick={() => {
                        setSelectedResource(m.resource);
                        setViewMode("market");
                      }}
                      className={`game-card rounded-lg p-3 text-left w-full transition-colors ${
                        isSelected
                          ? "border-brand/50 bg-brand/10"
                          : "bg-card border-border hover:border-muted-label"
                      }`}
                    >
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <div className="text-[9px] text-muted-label flex items-center gap-1">
                              <span className={sectorInfo.color}>
                                {sectorInfo.name.split(" ")[0]}
                              </span>
                              <span>·</span>
                        ...
                        <div className="flex items-center gap-1.5">
                          {activeInjection && (
                            <span
                              className={`text-[11px] px-1.5 py-0.5 rounded-full border font-bold inline-flex items-center gap-0.5 ${
                          ...
                          {priceRatio > 1.5 && (
                            <span className="text-[11px] px-1.5 py-0.5 rounded-full bg-domain/30 text-domain border border-domain/30 font-bold inline-flex items-center gap-0.5">
                              <GameIcon icon="game-icons:flame" size={10} /> HOT
                            </span>
                          )}
                          {priceRatio < 0.5 && (
                            <span className="text-[11px] px-1.5 py-0.5 rounded-full bg-brand/30 text-brand border border-brand/30 font-bold inline-flex items-center gap-0.5">
                              <GameIcon icon="game-icons:falling" size={10} />{" "}
                              LOW
                            </span>
                          )}
                          {m.trend === "up" && (
                            <span
                              className="text-xs trend-arrow-bounce inline-flex items-center"
                              style={{
                                filter:
                                  "drop-shadow(0 0 3px rgba(74,222,128,0.5))",
                              }}
                            >
                              <GameIcon
                                icon="game-icons:fast-arrow"
                                size={12}
                                className="text-success -rotate-90"
                              />
                            </span>
                          )}
                          {m.trend === "down" && (
                            <span
                              ...
                              style={{
                                filter:
                                  "drop-shadow(0 0 3px rgba(248,113,113,0.5))",
                              }}
                            >
                              <GameIcon
                                icon="game-icons:fast-arrow"
                                size={12}
                                className="text-danger rotate-90"
                              />
                            </span>
                          )}
                          {isAutoSell && (
                            <span className="text-[11px] px-1 py-0.5 rounded bg-success/30 text-success border border-success/30">
                              AUTO
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-1.5">
                          <span
                            className={`text-sm font-bold font-mono ${m.trend === "up" ? "text-success" : m.trend === "down" ? "text-danger" : "text-brand"}`}
                          >
                            ${m.currentPrice.toFixed(2)}
                          </span>
                          {m.trend === "up" && (
                        ...
                        <div className="text-[10px] text-muted-label">
                          ...
                          <span
                            className={`ml-1 inline-block w-8 h-1.5 rounded-full bg-muted-label align-middle overflow-hidden`}
                          >
                            <span
                      ...
                      {(prod > 0 || cons > 0) && (
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          {cons > 0 && (
                            <span className="text-[11px] text-domain font-mono">
                              -{cons.toFixed(1)}/s
                            </span>
                          )}
                        </div>
              ...
              <>
                ...
                <div className="game-card rounded-xl bg-card p-4 border border-brand/30">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm font-bold text-subtle">
                        {selectedMeta.name}
                      </h3>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span
                          className={`text-lg font-bold font-mono ${selected.trend === "up" ? "text-success" : selected.trend === "down" ? "text-danger" : "text-brand"}`}
                        >
                          ${selected.currentPrice.toFixed(2)}
                        </span>
                        <span
                          className={`text-xs font-mono ${priceChange >= 0 ? "text-success" : "text-danger"}`}
                        >
                          {priceChange >= 0 ? "+" : ""}
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        store.toggleAutoSell(selected.resource);
                      }}
                      aria-pressed={store.autoSellResources.includes(
                        selected.resource,
                      )}
                    ...
                    >
                      {store.autoSellResources.includes(selected.resource) ? (
                        <>
                          <GameIcon
                            icon="game-icons:spinning-wheel"
                            size={14}
                            className="inline"
                          />{" "}
                          AUTO
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    ...
                    <PanelStatCard
                      ...
                      subtext={
                        RESOURCE_ELASTICITY[selected.resource] > 0.5
                          ? "Volatile"
                          : "Stable"
                      }
                      color={
                  ...
                  {(playerImpact.production > 0 ||
                    playerImpact.consumption > 0) && (
                      ...
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div>
                          ...
                          <div className="text-xs text-success font-mono font-bold">
                            +{playerImpact.production.toFixed(1)}/s
                          </div>
                        </div>
                        <div>
                          <div className="text-[10px] text-muted-label">
                          ...
                          <div className="text-xs text-domain font-mono font-bold">
                            -{playerImpact.consumption.toFixed(1)}/s
                          </div>
                        </div>
                        <div>
                          <div className="text-[10px] text-muted-label">
                          ...
                          <div
                            className={`text-xs font-mono font-bold ${playerImpact.netPressure > 0 ? "text-danger" : playerImpact.netPressure < 0 ? "text-success" : "text-subtle"}`}
                          >
                            {playerImpact.netPressure > 0
                              ? "↓ Pushes down"
                              : playerImpact.netPressure < 0
                                ? "↑ Pushes up"
                                : "─ Neutral"}
                        ...
                        {correlationChain.upstream.slice(0, 3).map((c) => (
                          ...
                          >
                            <span className="text-domain">
                              ↑ {RESOURCE_META[c.resource].name}
                            </span>
                            <span className="text-muted-label font-mono">
                              {(c.strength * 100).toFixed(0)}%
                            </span>
                          </div>
                        ))}
                ...
                <div className="game-card rounded-xl bg-card p-4 border border-success/30">
                  ...
                  <div className="flex items-center gap-1 mb-3">
                    ...
                    <button
                      onClick={() =>
                        setTradeAmount(tradeMode === "sell" ? maxSell : maxBuy)
                      }
                      className={`flex-1 py-1.5 rounded text-[10px] font-mono ${
                        tradeAmount ===
                        (tradeMode === "sell" ? maxSell : maxBuy)
                          ? "bg-brand/30 text-brand border border-brand/30"
                          : "bg-background text-muted-label border border-muted-label hover:text-subtle"
                      }`}
                    >
                  ...
                  <div className="space-y-1.5 mb-3 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="text-success font-mono">
                        ${(selected.currentPrice * 0.9).toFixed(2)}/u
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-label">Buy Price</span>
                      <span className="text-domain font-mono">
                        ${(selected.currentPrice * 1.1).toFixed(2)}/u
                      </span>
                    </div>
                      ...
                      {tradeMode === "sell" ? (
                        <div className="flex justify-between">
                          <span className="text-muted-label">
                            Sell {tradeAmount} for
                          </span>
                          <span className="text-success font-mono font-bold">
                            ${formatNumber(sellRevenue)}
                          </span>
                        </div>
                      ) : (
                        <div className="flex justify-between">
                          ...
                          <span
                            className={`text-domain font-mono font-bold ${store.money < buyCost ? "text-danger" : ""}`}
                          >
                    ...
                    {tradeMode === "sell" && tradeAmount > 100 && (
                      <div className="text-[9px] text-warning flex items-center gap-1">
                        <Flame className="w-3 h-3" /> Large sell may depress
                        market price
                      </div>
                    )}
                    {tradeMode === "buy" && tradeAmount > 100 && (
                      <div className="text-[9px] text-warning flex items-center gap-1">
                        <Flame className="w-3 h-3" /> Large buy may inflate
                        market price
                      </div>
                    )}
                    {tradeMode === "sell" && tradeAmount > maxSell && (
                      <div className="text-[9px] text-danger">
                        Only {maxSell} available to sell
                      </div>
                    )}
                    {tradeMode === "buy" && tradeAmount > maxBuy && (
                      <div className="text-[9px] text-danger">
                        Can only afford/buy {maxBuy}
                      </div>
                    )}
                  </div>
            ...
            <div className="game-card rounded-xl bg-card p-4 border border-warning/20">
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className="border-warning/30 text-warning bg-warning/10 text-[9px]"
                  >
                ...
                <div className="text-center py-8">
                  ...
                  {(store.marketNews?.length ?? 0) === 0 && (
                    <p className="text-[10px] text-muted-label mt-1">
                      Start producing and trading to generate market activity.
                    </p>
                  )}
                </div>
              ) : (
                    ...
                    const severityBorder =
                      news.severity === "high"
                        ...
                        : news.severity === "medium"
                          ? "border-l-warning/50"
                          : "border-l-muted-label/40";
                    const style = getSeverityStyle(news.severity);
                    const isLLM = news.textSource === "llm";
                    return (
                      <div
                        key={news.id}
                        className={`rounded-lg p-3 border ${style.border} ${style.bg} border-l-2 ${severityBorder}`}
                      >
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                              <span className="text-[9px] text-muted-label">
                                {getCategoryIcon(news.category)}{" "}
                            ...
                            {news.affectedResources.length > 0 && (
                              <div className="flex items-center gap-1 mt-1 flex-wrap">
                                {news.affectedResources.slice(0, 5).map((r) => (
                                  <span
                                    key={r}
                                    className="text-[11px] px-1.5 py-0.5 rounded bg-muted-label text-subtle border border-muted-label"
                                  >
                                ...
                                {news.affectedResources.length > 5 && (
                                  <span className="text-[11px] text-muted-label">
                                    +{news.affectedResources.length - 5} more
                                  </span>
                                )}
                              </div>
                            )}
          ...
          <div className="space-y-3">
              ...
              {(() => {
                return (
                  <div className="text-[10px] text-muted-label text-center py-3">
                    No active volatility injections
                  </div>
                );
              })()}
            </div>
            ...
            <div className="game-card rounded-xl bg-card p-4 border border-brand/20">
              ...
              {(store.marketNarratives?.length ?? 0) === 0 ? (
                <div className="text-[10px] text-muted-label text-center py-3">
                  Your actions will shape market narratives as you grow your
                  industrial empire.
                </div>
              ) : (
                <div className="space-y-2 max-h-64 overflow-y-auto game-scrollbar">
                  {(store.marketNarratives ?? []).map((narrative) => {
                    ...
                    return (
                      <div
                        key={narrative.id}
                        className={`rounded-lg p-2.5 border ${style.border} ${style.bg}`}
                      >
                        <div
                          className={`text-[10px] font-bold ${style.color} mb-0.5`}
                        >
                          {narrative.title}
                        </div>
                        ...
                        <div className="flex items-center gap-1 mt-1 flex-wrap">
                          <Factory
                            className="inline w-3 h-3 text-brand"
                            aria-hidden="true"
                          />{" "}
