    ...
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          <Badge variant="outline" className="text-[8px] px-1 py-0 border-emerald-500/50 text-emerald-400 bg-emerald-900/20 cursor-default">
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <Badge
          variant="outline"
          className="text-[10px] px-1.5 py-0 cursor-default border-emerald-500/50 text-emerald-400 bg-emerald-900/20"
        >
          <Users className="w-2.5 h-2.5 mr-1" />
          {onlineCount} online
          {loggedInCount > 0 && (
            <span className="ml-1 text-emerald-500/70">({loggedInCount} logged in)</span>
          )}
        </Badge>
      </TooltipTrigger>
      <TooltipContent side="bottom" className="bg-card border-cyan-900/30 w-56">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <p className="text-xs font-semibold text-emerald-300">Live Presence</p>
        </div>
