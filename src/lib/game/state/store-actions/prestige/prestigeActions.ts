// prestigeActions: the createPrestigeActions factory + both methods
// (doPrestige, purchasePrestigeBonus). Fire-and-forget leaderboard
// submission is delegated to submitPrestigeLeaderboard; the inline
// queueMicrotask wrapper stays here so the UI doesn't wait.

import { RANK_THRESHOLDS } from "../../../config/configCache";
import { soundEngine } from "../../../audio/soundEngine";
import { generateId } from "../../../shared/utils/generateId";
import type { SetFn, GetFn } from "../_actionTypes";
import { friendlyPrestigeError } from "./friendlyPrestigeError";
import { submitPrestigeLeaderboard } from "./leaderboardSubmission";
...
export function createPrestigeActions(set: SetFn, get: GetFn) {
  return {
    doPrestige: async () => {
      ...
      // shape spread (was the anti-pattern that drifted whenever the
      ...
      // reconstruct the canonical shape client-side any more — the
      // server owns it.
      ...
      queueMicrotask(() => {
        void submitPrestigeLeaderboard({
          corporationName,
          score,
          rankName,
          state,
          finalPrestigeCount: finalPrestigeState.totalPrestiges,
